...
<Tabs>
  ...
  **DeepSeek** is fully OpenAI-compatible with working `/models` auto-detection.
  ...
  | **Model IDs** | Auto-detected (e.g., `deepseek-v4-pro`, `deepseek-v4-flash`) |
  ...
  **Mistral AI** is fully OpenAI-compatible with working `/models` auto-detection.
  ...
  | **Model IDs** | Auto-detected (e.g., `mistral-large-3-25-12`, `mistral-small-4-0-26-03`, `codestral-25-08`) |

  :::caution No more `-latest` aliases
  Mistral no longer publishes `-latest` alias names such as `mistral-large-latest`. Use the dated model IDs from the [Mistral models overview](https://docs.mistral.ai/getting-started/models/models_overview/).
  :::
  ...
  **Groq** provides extremely fast inference via an OpenAI-compatible API.
  ...
  | **Model IDs** | Auto-detected (e.g., `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `groq/compound`) |
  ...
  **MiniMax** is a leading AI company providing high-performance coding-focused models. Their latest model, **MiniMax M3**, is optimized for agentic reasoning, tool use, coding, and long-context tasks. Their **Coding Plan** subscription is significantly more cost-effective for high-frequency programming than standard pay-as-you-go pricing.

  | Setting | Value |
  |---|---|
  | **URL** | `https://api.minimax.io/v1` |
  | **API Key** | Your Coding Plan API key (see Step 2 below) |
  | **Model IDs** | **Required**, add manually (e.g., `MiniMax-M3`) |

  :::note Screenshots below show `MiniMax-M2.5`
  The screenshots in this section were captured before M3 shipped. The steps are unchanged — substitute `MiniMax-M3` wherever you see `MiniMax-M2.5`.
  :::
  ...
  **Step 3: Configure Connection in Open WebUI**
  ...
  1. Open Open WebUI and navigate to **Settings > Admin > Connections**.
  ...
  5. In the **Model IDs (Filter)**, type `MiniMax-M3` and click the **+** icon.
  ...
  **Step 4: Start Chatting**
  ...
  Select **MiniMax-M3** from the model dropdown and start chatting. Reasoning and thinking work by default without any additional configuration.
  ...
  **Vercel AI Gateway** provides a single OpenAI-compatible endpoint for models across multiple providers, with Vercel-side routing, spend monitoring, and provider management.

  | Setting | Value |
  |---|---|
  | **URL** | `https://ai-gateway.vercel.sh/v1` |
  | **API Key** | Your AI Gateway API key from the [Vercel dashboard](https://vercel.com/docs/ai-gateway/authentication/api-keys) |
  | **Model IDs** | Auto-detected; optionally filter to specific gateway model IDs such as `anthropic/claude-opus-5` |
  ...
  **OpenRouter** aggregates hundreds of models from multiple providers behind a single API.
  ...
  1. **Use an allowlist**: add only the specific model IDs you need (e.g., `anthropic/claude-sonnet-5`, `google/gemini-3.6-flash`).
  2. **Enable model caching** via `Settings > Admin > Connections > Cache Base Model List` or `ENABLE_BASE_MODELS_CACHE=True`. Without caching, page loads can take 10-15+ seconds. See the [Performance Guide](/troubleshooting/performance) for more details.
  :::
  ...
  **Azure OpenAI** provides enterprise-grade OpenAI hosting through Microsoft Azure.
  ...
  To add an Azure OpenAI connection, set the **Provider** in the connection dialog:

  1. In the connection form, click **Advanced** to expand it.
  2. Set the **Provider** dropdown to **Azure OpenAI**.
  3. Fill in the settings below.

  | Setting | Value |
  |---|---|
  | **Provider** | **Azure OpenAI** (see [The Provider Setting](#the-provider-setting)) |
  ...
  :::tip Azure is detected from the URL
  If you leave **Provider** at **Default** and the URL contains `azure.` or `cognitive.microsoft.com` without ending in `/openai/v1`, the dialog switches itself into Azure mode and asks for the API Version and deployment names anyway. Setting the dropdown explicitly is only needed for Azure-compatible endpoints on other hostnames.
  :::

  :::tip Two Azure URL Formats
  ...
  **LiteLLM** is a proxy server that provides a unified OpenAI-compatible API across 100+ LLM providers (Anthropic, Google, Azure, AWS Bedrock, Cohere, and more). It translates between provider-specific APIs and the OpenAI standard.
  ...
  | **Provider** | `LiteLLM` (optional, see below) |

  Setting **Provider** to `LiteLLM` under **Advanced** only matters when something calls Open WebUI's own [Anthropic Messages endpoint](/reference/api-endpoints#-anthropic-messages-api), for example an Anthropic SDK or a coding agent pointed at Open WebUI. With it set, such a request is handed to LiteLLM untouched instead of being translated into Chat Completions and back, so Anthropic-only fields survive the trip. The trade is that the request goes straight to LiteLLM and skips the Open WebUI chat pipeline, so filters, tools and knowledge do not run on it. Normal chatting in the Open WebUI interface is unaffected either way.

  **Quick setup:**

  ```bash
  pip install litellm
  litellm --model gpt-5.6-sol --port 4000
  ```
...
<Tabs>
  ...
  **LM Studio** provides a local OpenAI-compatible server with a GUI for model management.
  ...
  | **Provider** | `LM Studio` (optional, enables model management from Open WebUI) |

  Start the server in LM Studio via the "Local Server" tab before connecting.

  Setting **Provider** to `LM Studio` under **Advanced** marks loaded models in the model selector and lets admins download, load and unload models without leaving Open WebUI. See [Managing Local Models from Open WebUI](#managing-local-models-from-open-webui).
...
:::info Forward cookies

**Advanced > Forward cookies** decides whether the cookies from the browser request are sent on to this server. It is off by default and applies to admin-configured connections; a user's own direct connections never forward cookies.

Turn it on only for a server you control that authenticates the person behind the request by cookie, typically one sitting inside your own network behind the same sign-on. Everything the browser holds for your Open WebUI domain goes with the request, session cookie included, so a third-party endpoint should never have this on.

**This changed:** connections using the **Session** or **System OAuth** auth types used to forward cookies unconditionally. They no longer do. If such a connection stops authenticating after an upgrade, switch this on.
...
| :--- | :--- |
| **Default** | Nothing. The connection is treated as a plain OpenAI-compatible endpoint. |
| **Azure OpenAI** | Requests are rewritten into Azure's deployment-based URL form with an `api-version` parameter, so the connection needs an **API Version** and the deployment names listed under **Model IDs**. Also switched on automatically for Azure-looking URLs, see the Azure OpenAI tab under [Cloud Providers](#cloud-providers). |
| **llama.cpp** | Unlocks the [model management panel](#managing-local-models-from-open-webui), the green **Loaded** marker and the admin **Eject** button in the model selector, and the extra model details on the model rows. Stored reasoning is also replayed to the server as a `reasoning_content` field, which llama.cpp routes through the chat template. See the [llama.cpp guide](/getting-started/quick-start/connect-a-provider/starting-with-llama-cpp) and [reasoning models](/features/chat-conversations/chat-features/reasoning-models). |
| **LM Studio** | Unlocks the same [model management panel](#managing-local-models-from-open-webui), loaded markers and model details, minus deleting models and minus the **Eject** button. |
| **LiteLLM** | Requests arriving at Open WebUI's own [Anthropic Messages endpoint](/reference/api-endpoints#-anthropic-messages-api) are forwarded to LiteLLM as-is instead of being converted to Chat Completions and back. Nothing changes for chats started inside Open WebUI. |

:::info Where the dropdown is not shown
...
| | `llama.cpp` | `LM Studio` |
| :--- | :---: | :---: |
| List the server's models | Yes | Yes |
| Download a model by ref | Yes | Yes |
| Load / unload a model | Yes | Yes |
| Delete a cached model | Yes | No |
