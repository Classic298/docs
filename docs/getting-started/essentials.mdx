...
5. [**Tool calling** — letting the model do things, not just talk](#-tool-calling-letting-the-model-do-things)
...
Open WebUI has two tool-calling modes: **Default** (older, prompt-based) and **Native** (uses the provider's own tool/function-calling API). Native is faster, more reliable, doesn't eat into the context window with long tool-use instructions, and preserves KV cache. If your model supports it (all mainstream ones do — OpenAI, Anthropic, Gemini, Llama 3.1+, Qwen 2.5+, DeepSeek, and more), turn it on:

- Per-model: **Workspace → Models → [your model] → Advanced Params → Function Calling = Native**
- Per-chat override: in a chat's **Chat Controls** (right sidebar)

If a tool "isn't being called" on a capable model, 90% of the time Native mode just needs flipping on.
