...
- Type: `str`
- Default: Empty string (' ')
- Description: Comma-separated list of model IDs to pin by default for users who haven't pinned any models of their own. This gives such accounts a ready-made set of model shortcuts in the sidebar. It is resolved for display only: nothing is written to a user's own settings until they pin, unpin or reorder something, and from then on their pins win. In the interface this list is managed per model as **Set as Pinned Model** in **Settings > Admin > Models**, see [Selected and Pinned Models](/features/workspace/models#selected-and-pinned-models-admin).
...
It bounds **both** paths Open WebUI offloads on: the AnyIO limiter behind `run_in_threadpool`, and the event loop's default executor behind `asyncio.to_thread`, which carries most of the blocking work, vector database calls, storage reads, LDAP binds and password verification among it. The two are independent pools and each gets this ceiling, so the per-process thread ceiling is twice the value. Both fill lazily, so a ceiling only becomes threads when that many blocking operations are genuinely in flight at once.
...
- **Exception, weak hardware (Raspberry Pi, tiny VPS, containers capped at ~250m CPU / very low RAM):** do **not** set `2000` here. Each *genuinely concurrent* blocking op still uses a real OS thread (stack memory), and the ceiling is applied to each of the two pools, so on a tiny device an enormous ceiling lets a traffic burst spawn enough threads to exhaust RAM. Leave it at the default, or set a modest value (e.g. a few hundred) matched to what the device can actually absorb. This caveat applies only to constrained single-board / micro deployments; any normal server should use `2000+`.

:::

:::info Size it from the narrowest thing downstream, not from CPU
Threads are not the scarce resource, and CPU count is not what to size against: this work is waiting on I/O rather than computing. Each offloaded call wants something downstream instead, a database connection, a vector store connection, a slot at an embedding endpoint, and that is what runs out first.

A bounded database pool absorbs the pressure rather than passing it on. Checkouts are capped per process, by `DATABASE_POOL_SIZE` + [`DATABASE_POOL_MAX_OVERFLOW`](#database_pool_max_overflow) where you set them and by SQLAlchemy's own defaults where you do not, so a thread that cannot get a connection waits at the pool and the pool stays the ceiling however many threads exist. [`DATABASE_POOL_SIZE=0`](#database_pool_size) is the case to watch: it disables pooling, every checkout opens its own connection, and the thread ceiling becomes the connection ceiling.

Destinations with no queue behave differently: an embedding endpoint that rate-limits returns errors rather than making callers wait, so size against the narrowest of those. The ceiling is per process, so multiply by the replicas or containers you run before comparing it with anything they share.
:::

#### `THREAD_POOL_THREAD_NAME_PREFIX`

- Type: `str`
- Default: Empty string (Python's own `ThreadPoolExecutor-N` naming)
- Description: Prefix applied to the names of the threads in the event loop's default executor, the pool [`THREAD_POOL_SIZE`](#thread_pool_size) sizes. It is cosmetic: it exists so Open WebUI's offload threads can be told apart in thread dumps, profilers and any monitoring that groups by thread name. It has no effect unless `THREAD_POOL_SIZE` is set, since that is the only case where the executor is replaced.
...
- Type: `bool`
- Default: `False`
- Description: Bypasses model access control. When set to `true`, all users (and admins alike) will have access to all models, regardless of the model's privacy setting (Private, Public, Shared with certain groups). This is useful for smaller or individual Open WebUI installations where model access restrictions may not be needed. It governs which models a user may reach, not which backend serves them: on a proxied route that takes a connection index, the index a non-admin supplies must still be one that actually serves the requested model, and this setting does not waive that check.
...
:::info The page's own requests come from Open WebUI, not the browser
The loader intercepts everything the page asks for and fetches it through Open WebUI's own HTTP clients, then hands the result back to the browser. With a remote browser this means the traffic leaves from the Open WebUI host rather than the browser host, so egress rules, allowlists and outbound proxies have to permit the former. It also means those requests are verified against `certifi` and [`AIOHTTP_CLIENT_SSL_CERT_FILE`](#aiohttp_client_ssl_cert_file) rather than the browser image's own trust store, so a site behind an internal certificate authority needs that authority trusted here.
:::
