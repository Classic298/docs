...
- Type: `bool`
- Default: `False`
- Description: Enables the SCIM 2.0 endpoints at `/api/v1/scim/v2`, so an identity provider can create, update and deactivate users and groups in Open WebUI automatically. Included in the free edition with no licence key and no user limit. Also accepts the older name `SCIM_ENABLED`.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **private sharing** of notes. Parent of [`USER_PERMISSIONS_NOTES_ALLOW_PUBLIC_SHARING`](#user_permissions_notes_allow_public_sharing).
- Persistence: This environment variable is a `ConfigVar` variable.
