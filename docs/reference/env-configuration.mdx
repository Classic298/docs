...
The OAuth settings are the exception: they are shown read-only in the Admin Panel whenever [`ENABLE_OAUTH_PERSISTENT_CONFIG`](#enable_oauth_persistent_config) is `False`, which is its default, so there is nothing to edit and nothing to lose.
...
- Type: `bool`
- Default: `False`
- Description: Allows the **first** account to be created through `POST /api/v1/auths/signup` on an instance whose login form is turned off. It applies only while no user exists; once there is one, ordinary signup rules take over and this has no further effect. Without it, an instance configured for SSO only (`ENABLE_LOGIN_FORM=false`) has no way to create its first administrator through the API, which is awkward after a database reset. Leave it off unless you are bootstrapping such an instance, and turn it off again afterwards.
...
- Type: `bool`
- Default: `True`
- Description: Shows the email and password fields, the sign-in button and the "or" divider (the divider only when `ENABLE_OAUTH_SIGNUP` is on) on the login page. Turning it off also refuses local sign-ups at `POST /api/v1/auths/signup`, so an SSO-only instance cannot create accounts that way; see [`ENABLE_INITIAL_ADMIN_SIGNUP`](#enable_initial_admin_signup) for bootstrapping the first administrator. Password sign-in through the API is unaffected, that one follows `ENABLE_PASSWORD_AUTH`. Set up SSO or LDAP before turning this off, or nobody can get in.
- Also a toggle: **Login Form** in **Settings > Admin > Authentication**, which applies immediately and needs no restart.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: Empty string, which lets the browser's language pick the locale, with `en-US` as the fallback when no translation matches
...
- Description: Sets a default Language Model. Accepts several IDs separated by commas, which pre-selects that set of models on a new chat. Models that no longer exist, or that are hidden, are dropped from a new chat's selection and replaced by these defaults, so a user whose last-used model has been removed still starts on a working one rather than an empty selector. A user's own default model, and a `model` or `models` [URL parameter](/features/chat-conversations/chat-features/url-params), both take precedence and are left alone. In the interface this list is managed per model as **Set as Selected Model** in **Settings > Admin > Models**, see [Selected and Pinned Models](/features/workspace/models#selected-and-pinned-models-admin).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str` (JSON array)
- Default: `[]`
- Description: Sets the order models are listed in, as a JSON array of model IDs, for example `["gpt-5.6-sol","qwen3.6:27b"]`. Models named here appear first in that order, and anything left out follows behind. Previously this could only be arranged in the admin interface, which meant the arrangement was lost on restart wherever configuration is not persisted, so setting it here makes the order survive. Invalid JSON is logged and ignored rather than stopping startup.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: Empty string (' ')
- Description: Comma-separated list of model IDs to pin by default for users who haven't pinned any models of their own. This gives new accounts a ready-made set of model shortcuts in the sidebar. It seeds the user's own pin list once; after that the user's pins win. In the interface this list is managed per model as **Set as Pinned Model** in **Settings > Admin > Models**, see [Selected and Pinned Models](/features/workspace/models#selected-and-pinned-models-admin).
- Example: `gpt-5.6-sol,claude-opus-5,muse-glimmer:30b-q4_K_M`
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `dict` (JSON object)
- Default: `{}`
- Description: Sets global default parameters (temperature, top_p, max_tokens, seed, etc.) for all models. These defaults are applied as a baseline at chat completion time. Per-model parameter overrides always take precedence. Configurable via **Settings > Admin > Models** (**Model Defaults** > **Configure** > **Model Parameters**).
...
- Both this and a model's saved parameters only fill in what a request left out, so a parameter sent in the request body keeps the value the caller sent
...
- Type: `dict` (JSON object)
- Default: `{}`
- Description: Sets instance-wide starting values for the personal options in **Settings > Interface**, so every account opens on your preferences instead of the built-in ones, for example `{"chatBubble": false, "widescreenMode": true, "chatHoverPreview": false}`. A user's own choice always wins. An option nobody has touched keeps following the default, so changing it later moves everyone who has not overridden it. The defaults also reach personal settings written outside the **Interface** page, such as `system` and `params`, which the admin editor does not offer. Configurable via **Settings > Admin > General > UI > Default Interface Settings**. See [Default Interface Settings](/features/administration/interface-defaults) for the full list of keys, their labels and their built-in values.
- Persistence: This environment variable is a `ConfigVar` variable. Stored at config key `ui.default_interface_settings`.

:::info

`DEFAULT_INTERFACE_SETTINGS` is read from the environment as a JSON string at startup.

- Use valid JSON (for example: `{"chatBubble": false, "title": {"auto": false}}`)
- If parsing fails, or the value is not a JSON object, it is ignored and no defaults are applied
...
- Type: `str`
- Options: `thread` (default), `channel`
- Default: `thread`
- Description: Where a model's reply goes when it is mentioned in a message posted at the top level of a channel. `thread` starts a thread under that message, keeping a long answer out of the main timeline. `channel` posts the reply straight into the channel, where everyone reads it without opening anything. A mention **inside** an existing thread always answers in that thread, whichever mode is set. Can also be set in **Settings > Admin > General** (**Model Response Mode**), which appears once channels are enabled; an unrecognised value is ignored and the current setting is kept.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > General > Folder Max File Count**. Default is none (empty string) which is unlimited.
...
- Type: `bool`
- Default: `True`
- Description: When the [memory feature](/features/chat-conversations/memory) is on, controls whether stored memories are injected into the model's system context. Set to `False` to keep the memory tools available (the model can still add, update and delete memories) while no longer adding remembered context to the prompt. Injection additionally requires the user to hold the `features.memories` permission ([`USER_PERMISSIONS_FEATURES_MEMORIES`](#user_permissions_features_memories); admins are exempt). Surfaced as the "Memory System Context" toggle under **Settings > Admin > General** when Memories is enabled.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- **Compliance posture**: meeting requirements (especially in jurisdictions with stronger labour-protection law, e.g. DE / AT / EU) that admins not be *casually* presented with other users' data, even though they remain technically able to reach it via the routes above.
...
Treat anything in this cluster as *what the admin sees and does in the product UI and API*, not *what the admin technically can reach on the deployment*.
...
- Description: Controls whether admin users see other users' workspace items in **list and selector UI surfaces** (workspace tabs for models / knowledge / prompts / tools / notes / skills, the chat model selector, the file browser list, etc.). When set to `True` (default), those surfaces show every item from every user, convenient for single-admin / small-team deployments. When set to `False`, those same surfaces show only the admin's own items plus items explicitly shared with them, matching what a regular user would see.
  ...
  The same flag decides whether admins bypass access grants on admin-configured resources, which are external tool servers, MCP servers, Open Terminal connections, and arena models. With `True`, admins see and use all of them. With `False`, admins are checked against each resource's access list, except that a resource with no grants at all stays admin-only, since it has no owner to fall back to. See [What Private Means for Each Resource Type](/features/authentication-access/rbac/groups#what-private-means-for-each-resource-type).
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables user webhooks (each user can set a URL their notifications are POSTed to). For SSRF safety, webhook URLs that resolve to private or reserved IPs are blocked unless [`ENABLE_RAG_LOCAL_WEB_FETCH`](#enable_rag_local_web_fetch) is `true`.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: Empty string
- Description: Prefix for the names of the worker threads in the pool sized by [`THREAD_POOL_SIZE`](#thread_pool_size), so they can be told apart from other threads in thread dumps and profilers. Empty keeps Python's default `ThreadPoolExecutor-N_M` naming.
...
- Type: `bool`
- Default: `True`
- Description: Controls the user status feature: the emoji, message and expiry a user sets on their own account to tell other people what they are up to. Enabled, the control sits in the user menu and a status set there is shown to others beside that user. Disabled, the control is gone and the two endpoints behind it (`GET /api/v1/users/user/status` and `POST /api/v1/users/user/status/update`) answer `403`.
- Persistence: This environment variable is a `ConfigVar` variable. It can be toggled in the **Settings > Admin > General > User Status**.
...
- Type: `str`
- Default: Empty string
- Description: Free-form label for this deployment. It is returned as `deployment_id` by `GET /api/version`, so clients and monitors can tell deployments apart.

#### `INSTANCE_ID`

- Type: `str`
- Default: A random UUID generated at each start
- Description: Identifies the backend process. It is stamped as `instance_id` on every event envelope Open WebUI emits, so a multi-replica deployment can see which replica produced an event. Set it explicitly to keep the value stable across restarts.
...
- Type: `bool`
- Default: `False`
- Description: Lets the text of a streamed response grow in place as chunks arrive, instead of copying the whole answer so far on every chunk. The text that reaches the user is identical either way, and enabling it lowers peak memory, since the copy needs the old and the new string alive at the same time. The saving is worth having once answers get long: the copying cost grows with the square of the length, so a normal answer is sub-millisecond either way, while an answer of a few hundred thousand characters runs several times faster. That makes it a setting for busy instances and long outputs (agent transcripts, large code dumps, long tool output), where the work lands on the event loop and holds up every other stream on the worker. Turning it on is the faster option in every deployment that streams answers of that size; see [Scaling](/getting-started/advanced-topics/scaling) and [Performance](/troubleshooting/performance#-high-concurrency--network-optimization).
- Downside: the field is emptied while the chunk is attached and filled back in straight after, so an append that fails part way leaves that one message empty rather than holding the text so far. Reaching it means an allocation being refused rather than the kernel killing the process, which a container memory limit does not do, and the copying path needs more memory at that moment anyway, so it would fail first. No other code can observe the response mid-update.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Description: Sets the maximum buffer size in bytes for handling stream response chunks. When a single chunk exceeds this limit, the system returns an empty JSON object and skips subsequent oversized data until encountering normally-sized chunks. This prevents memory issues when dealing with extremely large responses from certain providers (e.g., models like gemini-3.1-flash-image or services returning extensive web search data). Set to an empty string, `0` or a negative value to disable chunk size limitations entirely; a value that is not a whole number is treated the same way. With no limit set (the default), a chunk of any size is passed through in full rather than breaking the response, at the cost of holding it in memory until it is complete. Recommended values are 16-20 MB (`16777216`) or larger depending on the image size of the image generation model (4K images may need even more). This handling applies to chat responses streamed from OpenAI-compatible connections, whichever **API Type** they are set to; Ollama connections never pass through it, so the value has no effect on them.

:::info

It is recommended to set this to a high single-digit to low double-digit number of megabytes (the value itself is counted in bytes, so `8388608` is 8 MB) if you run Open WebUI with high concurrency, many users, and very fast streaming models.
...
On reaching the cap, Open WebUI emits a **`Tool-call limit reached (N iterations).`** error in the chat. Before v0.9.6 it stopped silently (looked like the model "froze"); upgrading is the fix. A continuation request that fails outright, the provider erroring while the loop is fetching the model's next turn, is now reported the same way, as an error on the message rather than only a debug log line, so a run that ends early tells you why instead of just stopping. The same applies to the code interpreter's continuation loop. `256` is high enough for normal runs; raise it or set `-1` only for unbounded agents, accepting that a looping model then runs many more turns before stopping.
:::
...
- Type: `bool`
- Default: `False`
- Description: Set to `true` by the slim Docker image build. It marks a build without the local machine-learning stack: CUDA detection is skipped, [`VECTOR_DB`](#vector_db) defaults to `pgvector`, audio preprocessing with pydub is bypassed, and the evaluation leaderboard's local embedding model is not loaded.
...
- Type: `str`
- Default: `${DATA_DIR}/l.data`
- Description: Path of the license blob file read once at startup (Enterprise). When the file exists, its contents are loaded alongside [`LICENSE_KEY`](#license_key).

#### `LICENSE_PUBLIC_KEY`

- Type: `str`
- Default: Empty string
- Description: Body of the PEM public key used to verify the license (Enterprise), without the `BEGIN` and `END` lines. Leave unset unless Open WebUI has given you a value.
...
- Type: `bool`
- Default: `False`
- Description: Swaps the application-wide JSON encoder and decoder from the standard library's `json` module to [orjson](https://pypi.org/project/orjson/), covering HTTP request and response bodies, saving and opening chats, reading settings, the requests sent to model providers, upstream provider responses including streamed chunks, built-in tool results, the output a Jupyter server returns for code run in chat, signing in and signing up, Socket.IO payloads and the stored chunk details read back during knowledge base searches on the Valkey and Oracle 23ai vector stores. Off by default because orjson is stricter about what it accepts; rejected payloads fall back to the standard library automatically, as does anything encoded with a request orjson has no equivalent for, such as indented output or a custom rule for converting values before they are written. Worth enabling on clustered, Redis-backed deployments and not worth it on single-user instances. Read once at startup, so a restart is required. Added in v0.11.0.
- Details: [Multi-Replica → Use the Faster JSON Encoder](/troubleshooting/multi-replica#use-the-faster-json-encoder) for the full behaviour and trade-offs, [Scaling](/getting-started/advanced-topics/scaling#switch-the-json-encoder-to-orjson) for where it sits in a scaled rollout, and [Performance & RAM](/troubleshooting/performance#json-encoder) for how it compares to the other tuning knobs.
...
```
  ...
  Per-language versions of these suggestions are set in the admin panel rather than here, under **Settings > Admin > Models > Model Defaults**. See [Translations](/features/workspace/models#translations).
...
- Type: `int`
- Default: Empty string (`None`, no timeout)
...
When unset or empty, the timeout is `None` and the client waits indefinitely. A value that is not a
whole number falls back to `300`.
...
- Type: `int`
- Default: unset (no idle cap)
- Description: Maximum number of seconds a **streaming** request may go without receiving a chunk before it is aborted. This is an idle cap between chunks, not a cap on the whole response, so a long answer that keeps producing tokens is never cut off, only a stream that has gone silent is. `AIOHTTP_CLIENT_TIMEOUT` still bounds the total duration.

:::info

Leave it unset to keep the previous behavior, where a provider that stops sending data mid-stream holds the connection until the total timeout expires. Set it to something a little above your slowest expected gap between tokens (for example `60`) to fail fast on a stalled upstream. Non-numeric values and anything zero or below are treated as unset.
...
#### `AIOHTTP_CLIENT_ASYNC_DNS_RESOLVER`

- Type: `bool`
- Default: `False`
- Description: Chooses how Open WebUI turns hostnames into addresses for every outbound request. When `False` (the default), it asks the operating system, resolving names the way anything else on the machine does. When `True`, it uses the faster c-ares resolver, which stops lookups queueing behind each other on a busy instance. Read once at startup, so a restart is required.

:::warning Test it before you leave it on

c-ares reads `/etc/resolv.conf` and the hosts file but not other name sources, so `.local` names via avahi/mDNS, NIS or LDAP directories and Windows NBNS names may stop resolving, and on some Windows hosts it finds no usable nameserver at all. Exercise your models, web search and any internal services after turning it on, and turn it back off if lookups start failing.

The `aiodns` package is pinned to `3.6.1` for the same reason. If you build your own image, do not upgrade it past that pin while this setting is on.
...
- Description: Chunk size in bytes used when streaming files to and from external services, including audio transcription uploads, Ollama model blob uploads and downloads, and pipeline uploads. Files are sent and written in chunks of this size instead of being read into memory in full, so the value trades memory use against the number of round trips. Any value that is not a positive integer falls back to 1 MiB.
...
- Type: `str`
- Default: `INFO`
- Description: Sets the global logging level for all Open WebUI components. Valid values: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL`. The level also decides how much work the backend does: messages it filters out are never built, so raising it saves CPU as well as log volume, and the saving follows the size of the text that would have been discarded (whole request payloads, retrieval results). See [What the Log Level Costs](/getting-started/advanced-topics/logging#what-the-log-level-costs).
...
- Type: `bool`
- Default: `False`
- Description: Adds variable values to the tracebacks Open WebUI logs when an exception is caught. It is off by default because those annotations print the local variables at each frame, which in a request handler routinely means tokens, keys, prompts and user content ending up in your logs. Turn it on only while debugging a specific failure, on an instance whose logs you would treat as sensitive, and turn it off afterwards.
...
- Type: `bool`
- Default: `False`
- Description: Also audit `GET` requests. By default only the methods that change something (`PUT`, `PATCH`, `POST`, `DELETE`) are recorded, since reads are far more numerous and would dominate the log. Turn it on when you need to see who **read** what, accepting a much larger volume, and note that at `REQUEST_RESPONSE` level it means response bodies for every read as well.
...
:::info Context length: `num_ctx` overrides `OLLAMA_CONTEXT_LENGTH`
`OLLAMA_CONTEXT_LENGTH` is an **Ollama-server** setting, not an Open WebUI variable. Open WebUI's per-model `num_ctx` advanced parameter, when set, is sent on every request and **overrides** it, and the `num_ctx` control pre-fills a small `2048`. See [Context Length](/getting-started/quick-start/connect-a-provider/starting-with-ollama#context-length-num_ctx-and-ollama_context_length) for how to set this correctly and avoid blank responses with Native function calling.
:::
...
- Type: `str` (JSON)
- Default: `{}`
- Description: Per-connection configuration for the OpenAI-compatible backends, as a JSON object keyed by the connection's index in [`OPENAI_API_BASE_URLS`](#openai_api_base_urls) (its URL is also accepted as a legacy key). Each entry mirrors the per-connection settings from the admin UI, for example `enable`, `prefix_id`, `model_ids`, `tags`, and `connection_type`.
- Example: `{"0": {"enable": true, "prefix_id": "openai", "model_ids": ["gpt-5.6-sol"]}}`
- Persistence: This environment variable is a `ConfigVar` variable.
...
Several parameters have no effect on background requests wherever you set them: **Stream Chat Response**, **Stream Delta Chunk Size**, **Function Calling**, **Reasoning Tags** and **Context Compaction Threshold**. Emoji generation ignores the setting entirely.
...
- Type: `bool`
- Default: `False`
- Description: Adds a **Tool Permissions** entry to the chat input's ➕ menu, where a user picks **Full access** (run tools without asking, the default) or **Ask for approval** (stop before each tool call until it is allowed or denied). Experimental. While the setting is off, every request runs with full access and the `tool_approval_mode` parameter is ignored. Automations and channel replies always run with full access, since nobody is watching the chat to approve anything. Can also be toggled in **Settings > Admin > Interface**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables automatic context compaction. When a chat's estimated context exceeds `CONTEXT_COMPACTION_TOKEN_THRESHOLD`, older messages are summarized into a checkpoint and dropped from what is sent to the model, while recent messages are kept, so long conversations stay within the model's context window. Can also be toggled in **Settings > Admin > Interface**.
- Persistence: This environment variable is a `ConfigVar` variable.

#### `CONTEXT_COMPACTION_MODEL`

- Type: `str`
- Default: Empty string (follow the task model)
- Description: The model that writes context compaction summaries. Leave it empty to keep the previous behavior, where compaction uses the same model selection as title and tag generation ([`TASK_MODEL`](#task_model) / [`TASK_MODEL_EXTERNAL`](#task_model_external), falling back to the chat's own model). Set it to give summarization its own model, which is worth doing when your task model is a small one chosen for titles: a summary that will stand in for a large part of the conversation benefits from a more capable model than a five-word title does. A model ID that is not available falls back to the task model selection, and then to the chat model, so a stale value never blocks compaction. Can also be set in **Settings > Admin > Interface** (**Context Compaction Model**), where **Current Model** means follow the chat.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: The absolute ceiling, in tokens, for any per-model context compaction threshold. A model can raise or lower its own threshold with the `compact_token_threshold` advanced parameter, but the effective value is clamped to this cap. When unset it equals `CONTEXT_COMPACTION_TOKEN_THRESHOLD`, reproducing the previous behaviour in which per-model thresholds could only be lowered. Can also be set in **Settings > Admin > Interface** (**Token Cap**).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `int`
- Default: `40`
- Description: The percentage of the most recent messages kept verbatim when context compaction runs; everything older than that is replaced by the summary. Values are clamped to the range `10` to `50`, so at least a tenth and at most half of the conversation is retained in full. Compaction always splits on a user message, so the retained portion begins at a turn boundary rather than mid-exchange. Can also be set in **Settings > Admin > Interface** (**Retained Messages**).
- Persistence: This environment variable is a `ConfigVar` variable.
...
:::info Legacy feature
Code execution and both of its engines are **legacy**. `pyodide` (browser/WebAssembly) and `jupyter` (server) are legacy execution engines, so every variable in this section is legacy configuration. For full Python and shell access, use [Open Terminal](/features/open-terminal) instead, which runs code in an isolated container. The variables below still work and are documented for existing deployments.
:::
...
- Type: `str`
- Default: `pyodide`
- Description: Specifies the code execution engine to use. `pyodide` (browser) and `jupyter` (server) are **legacy** engines; for full Python and shell access use [Open Terminal](/features/open-terminal) instead.
- Persistence: This environment variable is a `ConfigVar` variable.
...
:::info Legacy feature
The code interpreter runs on the same **legacy** `pyodide` and `jupyter` engines as [Code Execution](#code-execution) above, so every variable in this section is legacy configuration. New deployments that need code or shell execution should use [Open Terminal](/features/open-terminal). The variables below still work and are documented for existing deployments.
:::
...
- Type: `str`
- Default: `pyodide`
- Description: Specifies the code interpreter engine to use. `pyodide` and `jupyter` are **legacy** engines; for full Python and shell access use [Open Terminal](/features/open-terminal) instead.
- Persistence: This environment variable is a `ConfigVar` variable.

#### `CODE_INTERPRETER_BLOCKED_MODULES`

- Type: `str` (comma-separated list of module names)
- Default: Empty string (nothing blocked)
- Description: A comma-separated list of Python module names the code interpreter refuses to import. Empty by default, in which case the blocking hook is never installed and every module is importable. When set, it intercepts only **direct, top-level `import <name>` statements in the executed code's main scope**; it does not remove the modules from the runtime, so imports reached indirectly (through a helper module, `importlib`, `__import__`, or `sys.modules`) still succeed. Treat it as an opt-in convenience filter for discouraging casual use of specific modules, not as a security boundary: the actual isolation is the execution engine, the browser sandbox for `pyodide` or your own server for a `jupyter` kernel, not this list.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `str`
- Default: The value of [`CODE_EXECUTION_JUPYTER_URL`](#code_execution_jupyter_url), otherwise empty
...
- Type: `str`
- Default: The value of [`CODE_EXECUTION_JUPYTER_AUTH`](#code_execution_jupyter_auth), otherwise empty
...
- Type: `str`
- Default: The value of [`CODE_EXECUTION_JUPYTER_AUTH_TOKEN`](#code_execution_jupyter_auth_token), otherwise empty
...
- Type: `str`
- Default: The value of [`CODE_EXECUTION_JUPYTER_AUTH_PASSWORD`](#code_execution_jupyter_auth_password), otherwise empty
...
- Type: `int`
- Default: The value of [`CODE_EXECUTION_JUPYTER_TIMEOUT`](#code_execution_jupyter_timeout), otherwise `60`
...
#### `ENABLE_DIRECT_INTEGRATIONS`

- Type: `bool`
- Default: `False`
- Description: Controls whether users reach their personal **Integrations** tab in **Settings**, where they add their own tool servers. Off by default, so the tab is out of sight until an admin turns it on under **Settings > Admin > Integrations**, in the **User Connections** section. Turning it on shows the tab to admins and to users holding the `direct_tool_servers` permission; the permission alone is no longer enough. Connections that already exist keep being used while the tab is hidden, so switching this off takes the entry point away without breaking anything already wired up.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str` (JSON)
- Default: `[]`
- Description: Defines the evaluation arena models as a JSON list of objects. Used to seed the arena model configuration at startup, mirroring what can be configured in the admin evaluation settings. List every ID in an object's `meta.model_ids` once: with the default `include` filter mode one of them is drawn at random per message, and an ID that appears twice is drawn twice as often. Editing such an arena model in [Settings > Admin > Evaluations](/features/administration/evaluation/#configuring-an-arena-model) and saving it collapses the duplicates.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: SentenceTransformers model the evaluation leaderboard loads to weight each feedback entry by how closely its tags match the leaderboard's query filter. It is not loaded on the slim image, where the query filter leaves the rankings unweighted.
...
- **Cache Refresh**: The cache is refreshed when:
  ...
  - The connection settings are saved in the **Settings > Admin > Connections** (clicking the **Save** button on the bottom right will trigger a refresh and update the cache with the newly fetched models)
...
- Persistence: This environment variable is a `ConfigVar` variable. It has no toggle in the interface, so on an instance that has already started once the environment variable is ignored and the stored value wins. Change it by posting the new value to `/api/v1/configs/import` with an admin token, or set [`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) to `False` to have environment variables win on every start.
...
- Type: `int`
- Default: `2000`
- Description: Maximum number of characters of `context`-type memories injected into the prompt. Anything beyond this budget is cut off.
- Persistence: This environment variable is a `ConfigVar` variable. It has no toggle in the interface, so on an instance that has already started once the environment variable is ignored and the stored value wins. Change it by posting the new value to `/api/v1/configs/import` with an admin token, or set [`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) to `False` to have environment variables win on every start.
...
| Header | Forwarded to |
|---|---|
| `X-OpenWebUI-User-Name`, `X-OpenWebUI-User-Id`, `X-OpenWebUI-User-Email`, `X-OpenWebUI-User-Role` | OpenAI, Ollama and Anthropic model connections, tool servers (OpenAPI and MCP), plus external embedding, reranking, web-search, audio, image and document-loader services (including Mistral OCR) |
...
| `X-OpenWebUI-Auth-Type` | OpenAI and Ollama model connections only, on model listing and chat requests |

So an OpenAI, Ollama or Pipelines request receives **five** of the six headers (the four user headers plus `X-OpenWebUI-Chat-Id`); only `X-OpenWebUI-Message-Id` is tool-server-specific, since it is what powers [external tool event emitting](/features/extensibility/plugin/development/events#-external-tool-events).

`X-OpenWebUI-Auth-Type` says how the caller authenticated to Open WebUI: `jwt` for a browser session, `api_key` for a request made with an API key. An upstream can use it to treat automation differently from a person at the keyboard, refusing a key-driven request to an expensive model, for example. Rename it with [`FORWARD_USER_INFO_HEADER_AUTH_TYPE`](#forward_user_info_header_auth_type).

It rides along on OpenAI and Ollama connections only. Tool servers, the RAG and document-extraction backends, web search, audio and image services do not receive it, so do not build an authorization rule on its absence.

If you set `FORWARD_USER_INFO_HEADER_JWT_SECRET`, the four `X-OpenWebUI-User-*` headers are replaced by a single signed JWT (see [`FORWARD_USER_INFO_HEADER_JWT_SECRET`](#forward_user_info_header_jwt_secret) below).

To forward **arbitrary custom headers**, including per-request values such as the message ID, set them on the individual connection instead of globally: see [Custom Forwarded Headers](#custom-forwarded-headers-per-connection) below.
...
- Type: `str`
- Default: `X-OpenWebUI-Auth-Type`
- Description: Renames the header carrying how the caller authenticated, `jwt` for a browser session or `api_key` for an API key. Only sent on OpenAI and Ollama connections, and only when `ENABLE_FORWARD_USER_INFO_HEADERS` is on.
...
#### Custom Forwarded Headers (per connection)
...
Set them in **Settings > Admin > Connections** by editing a connection and filling the **Headers** field with a JSON object, or in the connection's `headers` object inside [`OPENAI_API_CONFIGS`](#openai_api_configs) / [`OLLAMA_API_CONFIGS`](#ollama_api_configs). For example:

```json
{
  "X-Message-Id": "{{MESSAGE_ID}}",
  "X-User-Email": "{{USER_EMAIL}}",
  "X-Trace": "{{CHAT_ID}}/{{MESSAGE_ID}}"
}
```

Available placeholders, substituted at request time (any placeholder that has no value for the current request becomes an empty string):

| Placeholder | Value |
|---|---|
| `{{CHAT_ID}}` | Current chat/session ID |
| `{{MESSAGE_ID}}` | Current (assistant) message ID |
| `{{USER_MESSAGE_ID}}` | ID of the user message that triggered the request |
| `{{USER_MESSAGE_PARENT_ID}}` | Parent ID of that user message, i.e. its position in the message tree |
| `{{FILE_ID}}` | File ID, for file-scoped requests such as document processing |
| `{{FILE_NAME}}` | File name, for file-scoped requests |
| `{{FILE_CONTENT_TYPE}}` | File MIME type, for file-scoped requests |
| `{{TASK}}` | Internal task name when the request is a background task, e.g. title or tag generation |
| `{{USER_ID}}` | Signed-in user's ID |
| `{{USER_NAME}}` | Signed-in user's display name |
| `{{USER_EMAIL}}` | Signed-in user's email |
| `{{USER_ROLE}}` | Signed-in user's role |
| `{{AUTH_TYPE}}` | How the caller authenticated: `jwt` for a browser session, `api_key` for an API key |
| `{{USER_GROUPS}}` | Comma-separated names of the groups the user belongs to |
| `{{USER_GROUP_IDS}}` | Comma-separated IDs of the same groups |
| `{{USER_AGENT}}` | User-Agent of the originating browser request |

The two group placeholders cost a database lookup, so it is only performed when a header value actually contains one of them; every other request skips it. A user in no groups, or a lookup that fails, expands them to an empty string rather than failing the request.

Custom headers are honored by OpenAI, Ollama and direct model connections, tool servers (OpenAPI and MCP), and external document-processing connections. Introduced in v0.10.0, which expanded the placeholder set (the message, file, task and user-agent placeholders) beyond the earlier chat and user variables.
...
The same session cookie carries the authorization when a user [connects their account to an OAuth 2.1 MCP tool server](/features/extensibility/mcp#connecting-an-account), so `strict` breaks that as well: the provider's redirect arrives without the cookie and the connection fails with `OAuth callback failed: OAuth callback state is invalid or expired`.
...
- Type: `str`
- Default: `None`
- Description: URL the browser is sent to after signing out. The sign-out endpoint returns it as `redirect_url`, and for OpenID providers with an end-session endpoint it is passed along as `post_logout_redirect_uri`.
...
You do **not** need to manually set this variable for a secure installation. **All standard launch methods automatically generate and persist a random key on first start:**
...
| **Windows** (`backend\start_windows.bat`) | ✅ Yes | `.webui_secret_key` file next to the script |
| **Development** (`open-webui dev`, direct `uvicorn`) | ❌ No | N/A (set `WEBUI_SECRET_KEY` yourself) |

The Docker, `open-webui serve` and Windows methods check for the environment variable first, and if it is not set, they generate a random key, save it to a file and inject it into the environment, all before the application starts. That is why you do not need to set it yourself when you use them.

The Windows script builds its key from the command shell's own `%RANDOM%` generator, which is not a cryptographic source. That is adequate for a local install, but set `WEBUI_SECRET_KEY` explicitly on any host other people can reach.
...
- Description: Where the start script writes the auto-generated `WEBUI_SECRET_KEY` on first start, and reads it back from on every later start. A relative path is resolved against the `backend` directory the script lives in. Read by `start.sh` (Docker) and `backend\start_windows.bat` only; `open-webui serve` ignores it and always uses `.webui_secret_key` in the current working directory. It has no effect when `WEBUI_SECRET_KEY` is already set in the environment, because no key file is written or read in that case.

#### `WEBUI_SECRET_KEY_LENGTH`

- Type: `int`
- Default: `24`
- Description: How much randomness goes into an auto-generated `WEBUI_SECRET_KEY`. Only read on the first start, when no key is set in the environment and no key file exists yet.

:::warning The unit is not the same on every platform

`start.sh` and `open-webui serve` read it as a number of **bytes** and base64-encode them, so the default `24` gives a 32-character key. `backend\start_windows.bat` reads it as a number of **characters**, so `24` gives a 24-character key. Set it per platform if you need a specific length.

Avoid any value producing a key of exactly **44 characters** (`33` on Docker or `open-webui serve`, `44` on Windows), which stops Open WebUI starting for the reason given under [`ENABLE_VALVE_ENCRYPTION`](#enable_valve_encryption). The default is safe everywhere.
...
:::warning Keep `WEBUI_SECRET_KEY` stable
Encryption derives its key from [`WEBUI_SECRET_KEY`](#webui_secret_key). A key of any length is hashed into a valid Fernet key, with one exception: a key exactly 44 characters long is used verbatim and must be url-safe base64 decoding to 32 bytes. If you rotate `WEBUI_SECRET_KEY`, previously encrypted Valve values can no longer be decrypted and must be re-entered.
:::
...
- Type: `str`
- Options:
- `chroma`, `elasticsearch`, `mariadb-vector`, `milvus`, `opengauss`, `opensearch`, `pgvector`, `qdrant`, `pinecone`, `s3vector`, `oracle23ai`, `weaviate`, `valkey`
- Default: `chroma`, or `pgvector` on the slim image (where `USE_SLIM_DOCKER` is set)
...
:::warning Do not set this to a file path
Leave this variable unset if you want the embedded database (Milvus Lite). The default above already points at it, and setting the variable to that same path breaks startup.

The Milvus client library reads this variable from the environment itself, before Open WebUI is involved, and parses it as a network address. A file path fails that parse and the process exits during import with `Illegal uri`, naming the expected form `http[s]://[user:password@]example.com[:12345]`. Set the variable only when you are connecting to a Milvus server.

The parent directory must also exist before startup. The client opens the database file but does not create the directory holding it, and reports `Open local milvus failed, dir: ... not exists` if it is missing. This matters after any operation that clears the data directory, since the directory is not recreated for you.

Milvus Lite is embedded, so the file is held by a single process. A second process cannot open it while Open WebUI is running, which includes any attempt to inspect the database from a separate shell.
:::

:::danger Milvus Lite is not suitable for bulk document ingestion
Open WebUI checks a file's content hash against the collection before storing it, and Milvus Lite keeps no index on metadata, so that check reads the whole collection every time. Uploading a large corpus therefore gets slower with every file already stored. Keep the default ChromaDB or use a Milvus server for that. See [Swap ChromaDB for Milvus Lite](/troubleshooting/performance#4-swap-chromadb-for-milvus-lite-small-deployments) for the measurements.
:::
...
- Memories, and files that are not in a knowledge base, are NOT automatically migrated to the new collection schema and will appear missing
...
- Navigate to `Settings > Admin > Documents > Click Reindex Knowledge Base`

**This rebuilds knowledge base vectors into the new multitenancy collections, together with the per-file vectors of every file inside those knowledge bases**
**User memories, web search history and files that were never added to a knowledge base are NOT migrated by this operation**

**Verify knowledge bases are accessible and functional**
- Re-upload files that live outside any knowledge base if file-based retrieval for them is critical (file metadata remains but their vectors are not migrated)
...
- Vector DB reset from UI (Settings > Admin > Documents > Reset Vector Storage/Knowledge) only affects the active mode's collections
...
- Data loss risk: user memory vectors, and the vectors of files that are not in a knowledge base, are NOT migrated automatically. Reindexing migrates knowledge bases along with the per-file vectors of the files they contain.
...
- Understand that memories, and files outside knowledge bases, require re-upload/regeneration
...
- Navigate to `Settings > Admin > Documents`
...
:::info IVFFlat waits for rows before it builds
IVFFlat decides where its clusters sit by looking at the rows present when the index is built, and it needs 50 rows per list to place them, so 5,000 rows at the default of `100` lists. Open WebUI now waits for the `document_chunk` table to hold that many before creating the index, and logs that it is waiting. Searches run as an exact scan until then, which is correct and fast at that size.

An instance created on an earlier version built the index straight after the table, when it was empty, so its clusters were placed on nothing and every chunk added afterwards was filed against them. On a 20,000-row table that returns fewer than half the right chunks, recall at 10 of 0.480 against 1.000 once the index is built on real rows, at the same query time of about 5 ms. Repair it once with:

```sql
REINDEX INDEX idx_document_chunk_vector;
```

The build itself now happens on whichever startup crosses the threshold, so an instance that filled up in the meantime pays that cost once. `hnsw` builds as it always did and needs none of this.
:::
...
- Description: Tells pgvector to keep walking the index when a search has not yet found enough rows that match the filter. Every collection's chunks live in one table under one vector index, and the collection is filtered after the index walk, so without this a knowledge base holding a small share of the rows gets almost none of its real neighbours back: the index returns its globally nearest rows, and the filter throws nearly all of them away. On a 101,500-row table where the collection held 1.5% of the rows, recall at 10 measured **0.070 at 4.9 ms** with the scan off and **0.970 at 7.2 ms** with it on (hnsw, `m=16`). `strict_order` keeps results in exact distance order at more cost; `off` returns to the old behaviour.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).

:::info Applies only on pgvector 0.8 and newer
The setting is sent per search, and only when the installed pgvector extension is 0.8 or newer. On 0.7 it is skipped and noted in the log, because setting it there makes every search fail. `ivfflat` accepts only `relaxed_order`, so it is used whatever this is set to.
:::
...
Use [openGauss](https://opengauss.org/) as the vector store by setting `VECTOR_DB=opengauss`. Open WebUI connects to it with the PostgreSQL driver and creates its own table with a vector column on first use, so the server has to support vector columns and `ivfflat` indexes.

:::warning

openGauss is a non-core integration, not maintained by the core Open WebUI team. Fixes come from outside the core Open WebUI maintenance path.
If you want to use openGauss, be careful when upgrading Open WebUI (create backups and snapshots for rollbacks) in case internal changes in Open WebUI lead to breakage.
...
PostgreSQL Dependencies
openGauss uses the same driver as `pgvector`, so the PostgreSQL dependencies have to be installed:
...
- Type: `str`
- Default: The value of the `DATABASE_URL` environment variable
- Description: Sets the connection URL of the openGauss database that stores the embeddings, in the form `opengauss://user:password@host:port/dbname`. The scheme is what selects openGauss handling; a `postgresql://` URL is treated as a plain PostgreSQL server instead. The default is whatever `DATABASE_URL` is set to, so leave this unset only if `DATABASE_URL` is itself an `opengauss://` URL.

#### `OPENGAUSS_INITIALIZE_MAX_VECTOR_LENGTH`

- Type: `int`
- Default: `1536`
- Description: Sets the dimension of the vector column created on first use. Match it to your embedding model: embeddings longer than this are truncated and shorter ones are padded with zeros, both of which cost retrieval quality. The column keeps whatever dimension it was created with, so changing this afterwards does not resize an existing table.

#### `OPENGAUSS_POOL_SIZE`

- Type: `int`
- Default: `None` (no connection pooling)
- Description: Sets the number of connections to maintain in the openGauss connection pool. Pooling is only used when this is a positive number; the other `OPENGAUSS_POOL_*` variables are ignored otherwise.

#### `OPENGAUSS_POOL_MAX_OVERFLOW`

- Type: `int`
- Default: `0`
- Description: Specifies the maximum number of connections that can be created beyond `OPENGAUSS_POOL_SIZE` when the pool is exhausted.

#### `OPENGAUSS_POOL_TIMEOUT`

- Type: `int`
- Default: `30`
- Description: Sets the timeout in seconds for acquiring a connection from the openGauss pool.

#### `OPENGAUSS_POOL_RECYCLE`

- Type: `int`
- Default: `3600`
- Description: Specifies the time in seconds after which connections are recycled in the openGauss pool to prevent stale connections.
...
- Type: `int`
- Default: `5`
- Description: Sets the timeout in seconds for all requests made to the Qdrant server, so a slow or unresponsive Qdrant cannot hold a request open indefinitely.
...
`Reindex Knowledge Base` migrates knowledge bases and the per-file vectors of the files inside them. Files that were never added to a knowledge base, user memories and cached web search results stay in the old collections.
...
The chunk details stored with each vector are read back on every knowledge base search, which [`ENABLE_ORJSON`](#enable_orjson) makes faster.
...
Use [Valkey](https://valkey.io/) with the [valkey-search](https://github.com/valkey-io/valkey-search) module as the vector store by setting `VECTOR_DB=valkey`. Indexing, KNN search and metadata filtering all run server-side in Valkey. The chunk details stored with each vector are written on upload and read back on every knowledge base search, which [`ENABLE_ORJSON`](#enable_orjson) makes faster.
...
- Description: Sets the content extraction engine to use for document ingestion. See [Document Extraction](/features/chat-conversations/rag/document-extraction) for setup steps per engine, and [External Document Extraction](/features/chat-conversations/rag/document-extraction/external) for the `external` HTTP contract.
- Persistence: This environment variable is a `ConfigVar` variable.

#### `CONTENT_EXTRACTION_SUPPORTED_MEDIA_MIME_TYPES`

- Type: `str` (comma-separated MIME types, wildcards allowed)
- Default: unset
- Description: Which uploaded **images and videos** are handed to the content extraction engine instead of being stored without text extraction. Set it to something like `image/*` to let an OCR-capable engine read images, or name individual types (`image/png, image/jpeg`) to be selective. Leaving it unset keeps the old behavior: media goes to the extraction engine only when the engine is `external`, and every other engine stores images as-is and rejects unsupported types. Setting it to an empty list disables media extraction for every engine, `external` included. Video types count too, so `video/*` sends video uploads to the engine if yours can handle them; anything not matched is stored as-is, and an unmatched type that is neither image nor video is rejected as before. Audio is unaffected, since it follows the speech-to-text settings instead. Can also be set in **Settings > Admin > Documents** (**Supported Media MIME Types**).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `None`
- Description: Sets the URL for the external document loader service. See [External Document Extraction](/features/chat-conversations/rag/document-extraction/external) for the request/response contract your service needs to implement.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Options:
  ...
  - `4` - Tika 4.x
- Default: `3`
- Description: Tells Open WebUI which Apache Tika API to speak, since the two major versions differ in the endpoints they expose. Set this to match the server you are running, or extraction fails against an otherwise healthy Tika. It can also be changed from the Documents tab in the admin settings. On `4`, documents come back as Markdown, so headings, lists and tables survive extraction instead of arriving as flat text.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: Specifies all Docling processing parameters in JSON format. This is the primary configuration method for Docling processing options. All previously individual Docling settings are now configured through this single JSON object.
  ...
  **Supported Parameters:**
  ...
  - `vlm_pipeline_model_api` (str): Vision-language model API configuration. (e.g., `openai://gpt-5.6-sol`).

  **Note:** these values are forwarded to the Docling server as form fields exactly as entered, without re-encoding. Parameters whose value is a nested object (such as `picture_description_local` and `picture_description_api`) must therefore be written as JSON strings, for example `"picture_description_api": "{\"url\": \"...\", \"params\": {\"model\": \"llava:7b\"}}"`. Plain values, booleans and lists are entered directly.
...
```json
{
  ...
  "vlm_pipeline_model_api": "openai://gpt-5.6-sol"
}
```
...
- Type: `str`
- Default: Empty string
- Description: API key for the Datalab Marker service. Required when [`CONTENT_EXTRACTION_ENGINE`](#content_extraction_engine) is `datalab_marker`.
- Persistence: This environment variable is a `ConfigVar` variable.

#### `DATALAB_MARKER_API_BASE_URL`

- Type: `str`
- Default: Empty string, which uses `https://www.datalab.to/api/v1/marker`
- Description: Base URL of the Marker API. Point it at a self-hosted Marker server, or leave it empty for Datalab's hosted endpoint.
- Persistence: This environment variable is a `ConfigVar` variable.

:::info
Open WebUI posts to the `/marker` endpoint, which Datalab's API reference now marks as deprecated in favor of `/convert`. The `/convert` reference no longer lists `use_llm`, `force_ocr`, `strip_existing_ocr` or `format_lines`, but Open WebUI still sends the four settings below.
:::
...
- Type: `str`
- Default: Empty string
- Description: Extra Marker settings passed through unchanged as the `additional_config` JSON string. Datalab's reference lists `keep_pageheader_in_output`, `keep_pagefooter_in_output` and `keep_spreadsheet_formatting` as supported keys.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `use_llm`, which asks Marker to run an LLM pass for higher accuracy on tables, forms and math at higher cost and latency.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `skip_cache`, which makes Datalab re-run the conversion instead of returning a cached result for a file it has seen before.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `force_ocr`, which OCRs every page, including pages that already carry an extractable text layer.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `paginate`, which separates pages in the output with a horizontal rule carrying the page number.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `strip_existing_ocr`, which discards a document's existing OCR text layer and OCRs it again.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `disable_image_extraction`, which skips image extraction from the document.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Sends `format_lines`, which reformats lines with an OCR pass to improve inline math and text formatting.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `markdown`
- Description: Output format requested from Marker. Datalab accepts `markdown`, `html`, `json` and `chunks`. Open WebUI ingests the returned text, so keep `markdown` unless you have a reason not to.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `local`
- Description: `local` posts each file to a self-hosted MinerU server at [`MINERU_API_URL`](#mineru_api_url) and waits for the result. `cloud` uses MinerU's managed service, which uploads files in batches and polls for results, and requires [`MINERU_API_KEY`](#mineru_api_key).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: Base URL of the MinerU API: your self-hosted server in `local` mode, or MinerU's cloud API base in `cloud` mode.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: Empty string
- Description: API key for the MinerU cloud service. Required in `cloud` mode.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `int`
- Default: unset, in which case the limit follows `RAG_FILE_MAX_SIZE`, or no limit at all if that is unset too
- Description: Drops a metadata field from what is stored alongside a document chunk when the field is a list or an object whose text runs longer than this many characters. Extraction can attach large structures to a document, and those travel into the vector store with every chunk, where they cost memory and space without helping retrieval.

  Left entirely unbounded this is also a way to exhaust a server: an Office document is an archive, and one crafted to expand during extraction can turn a few hundred kilobytes into gigabytes of metadata held in memory, so a handful of uploads is enough to take an instance down. When this is unset the bound therefore follows the upload size limit, on the reasoning that a document cannot legitimately carry more metadata than the file itself is permitted to weigh. Set this explicitly to override that, and note that a deployment with no upload limit configured has no metadata limit either.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Description: Extra options passed to the **MinerU** document loader as a JSON object. Recognized keys include `enable_ocr` (default `false`), `enable_formula` (`true`), `enable_table` (`true`), `language` (`en`), and `model_version` (`pipeline`). Also configurable in **Settings → Admin → Documents** when MinerU is the content-extraction engine.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `""` (empty)
- Description: Authentication token for the PaddleOCR-vl server. Sent as `Authorization: token <value>` on every layout-parsing request. **The PaddleOCR-vl engine is skipped at runtime if this value is empty**: the loader falls back to the default loader chain for the current document even when `CONTENT_EXTRACTION_ENGINE=paddleocr_vl` is set. Set this to activate the engine.
- Persistence: This environment variable is a `ConfigVar` variable.

:::info Supported file types
As of v0.11.0, PaddleOCR-vl only receives `pdf`, `png`, `jpg`, `jpeg`, `bmp`, `tiff` and `webp` files. Every other extension (`txt`, `md`, `docx`, `xlsx`, `csv` and the rest) skips the engine entirely and goes to the default loader chain, so those files are still parsed and indexed, just by the built-in loaders. Size and provision your PaddleOCR-vl server for PDFs and images only. Before v0.11.0 all file types were sent to the server, and unsupported ones failed to index.

Of the files it does receive, images are dispatched with `fileType=1` and PDFs with `fileType=0`. Output is per-page Markdown, so downstream chunking behaves the same as other engines.
:::
...
- Type: `int`
- Default: `30000`
- Description: Caps the total output of a single `kb_exec` call, whatever command produced it. Output beyond the cap is cut and the model is told to narrow the command with a path, glob, `head`/`tail`/`sed` or `grep`. This is one budget for the whole pipeline, so a `cat` of a huge file and a broad `grep` are bounded the same way. Raise it if your models handle long tool output well and you would rather they read more per call; lower it to keep tool results from crowding the context window. Values below `1` fall back to the default.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `200`
- Description: How many files a single `kb_exec` `grep` may scan. Past this the command is refused with a suggestion to scope the search to a directory or glob rather than searching a whole knowledge base at once. Raise it for a deployment where broad searches over large knowledge bases are worth the time they take.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `50`
- Description: How many matching lines a text search returns before the rest are summarized as a count. Despite the name it governs every builtin grep: `grep_knowledge_files`, `kb_exec`'s `grep`, and `grep_chat_files` over the files attached to a chat, so all three behave alike.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `100000`
- Description: The hard ceiling, in characters, on what one `view_file` or `view_knowledge_file` call returns. A model asking for more than this is silently held to the ceiling; the response still carries pagination metadata, so it can continue from `next_offset`. `view_file` reads knowledge files and files attached to a chat alike, so this bounds both.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `int`
- Default: `10000`
- Description: How much `view_file` and `view_knowledge_file` return when the model does not ask for a size, bounded by [`VIEW_FILE_MAX_CHARS`](#view_file_max_chars), and applying to chat attachments as well as knowledge files. Raising it means fewer round trips through a long document at the cost of a larger first read.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `bool`
- Default: `False`
- Description: Controls what happens to a file when it is removed from a knowledge base, or when the knowledge base itself is deleted. By default the file record, the stored copy of the file and its embeddings are all deleted with it. Enable this to keep them, so that removal takes the file out of the knowledge base without destroying anything, which suits deployments that retain uploads for audit or reuse. Storage then grows with every removal, since nothing is reclaimed.
- Persistence: No
...
- Options:
  ...
  - `token_transformers`
- Default: `character`
- Description: Sets the text splitter for RAG models. `character` uses RecursiveCharacterTextSplitter. `token` uses TokenTextSplitter with the Tiktoken encoding named by `TIKTOKEN_ENCODING_NAME`. `token_transformers` splits by character boundaries but measures length with the HuggingFace tokenizer selected by [`RAG_TOKENIZER_MODEL`](#rag_tokenizer_model), and is unavailable on the slim image.
- Persistence: This environment variable is a `ConfigVar` variable.
...
#### `ENABLE_RAG_CSV_SUMMARY`

- Type: `bool`
- Default: `False`
- Description: Puts one line in front of every parsed `.csv` file naming its row count, its data row count and its column names, so a question about the size of the table is not answered from whichever rows happened to be retrieved. No summary is added when `CONTENT_EXTRACTION_ENGINE` is `external`, `tika` or `docling`, which parse `.csv` files themselves. Only files parsed after you turn it on get one, and re-indexing does not add it, so a CSV has to be re-uploaded. See the [RAG guide](/features/chat-conversations/rag#csv-table-summary).
- Persistence: Set via environment variable; changing it requires a restart (not a `ConfigVar`, so it does not appear in the admin settings).

#### `RAG_FILE_MAX_SIZE`

- Type: `int`
- Description: Sets the maximum size of a file in megabytes that can be uploaded for document ingestion. The limit applies to documents Open WebUI fetches itself as well, so a page or file retrieved from a URL is rejected once it exceeds the same size. Office and EPUB documents are checked a second time against their contents, since those formats are archives: a file whose contents unpack to far more than the file itself is refused rather than extracted, which stops a small upload from consuming the whole machine's memory. This value doubles as the default ceiling on extracted metadata, so leaving it unset removes that protection as well; see [`RAG_METADATA_MAX_VALUE_CHARS`](#rag_metadata_max_value_chars).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Description: Sets a model for reranking results. Locally, the model is loaded as a Sentence-Transformer CrossEncoder, with one exception: `jinaai/jina-colbert-v2` is loaded as a ColBERT model, and that takes precedence over `RAG_RERANKING_ENGINE`, so it runs locally even when the engine is set to `external`.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: The current name of [`ENABLE_RAG_LOCAL_WEB_FETCH`](#enable_rag_local_web_fetch), which it overrides when both are set. See that entry for what the gate covers.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- IPv6 private ranges
- IPv6 addresses that carry a private IPv4 address inside them, whatever the wrapper: IPv4-mapped (`::ffff:127.0.0.1`), IPv4-compatible (`::10.0.0.1`), 6to4 (`2002::/16`), Teredo (`2001::/32`), and NAT64 (`64:ff9b::/96` and `64:ff9b:1::/48`). These forms are globally routable as IPv6 while pointing at a loopback, RFC 1918 or metadata address, so each embedded address is extracted and checked on its own
...
When enabling `ENABLE_GOOGLE_DRIVE_INTEGRATION`, ensure that you have configured `GOOGLE_DRIVE_CLIENT_ID` and `GOOGLE_DRIVE_API_KEY` correctly, and have reviewed Google's terms of service and usage guidelines.
...
- Options:
  ...
  - `azure`: Uses [Azure AI Search](https://learn.microsoft.com/azure/search/). Requires `AZURE_AI_SEARCH_API_KEY`, `AZURE_AI_SEARCH_ENDPOINT` and `AZURE_AI_SEARCH_INDEX_NAME`.
  - `yacy`
  ...
  - `openserp`: Uses a self-hosted [OpenSERP](https://github.com/karust/openserp) instance. Requires `OPENSERP_BASE_URL`; no API key.
  - `firecrawl`: Uses [Firecrawl](https://firecrawl.dev/) for search. Requires `FIRECRAWL_API_KEY`; `FIRECRAWL_API_BASE_URL` points at a self-hosted instance.
  - `external`: Uses your own search endpoint. Requires `EXTERNAL_WEB_SEARCH_URL`; `EXTERNAL_WEB_SEARCH_API_KEY` authenticates the call.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Web Search > DDGS Backend** when DDGS is selected as the search engine.
...
- Description: Base URL of a self-hosted [OpenSERP](https://github.com/karust/openserp) instance, used when the web search engine is set to `openserp`. Open WebUI calls its `/mega/search` endpoint, which returns results from Google, Bing, Yandex, Baidu, DuckDuckGo and Ecosia together. No API key is involved, the base URL is the only required setting. See the [OpenSERP provider guide](/features/chat-conversations/web-search/providers/openserp).
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Web Search > OpenSERP URL** when `openserp` is selected as the search engine.
...
#### `SEARXNG_CLIENT_CERT_FILE`

- Type: `str`
- Default: unset
- Description: Path to a client certificate (PEM) presented when connecting to SearXNG, for an instance that requires mutual TLS. When set, Open WebUI builds a dedicated TLS context for SearXNG requests instead of using the shared outbound one; other outbound requests are unaffected and keep following [`AIOHTTP_CLIENT_SESSION_SSL`](#aiohttp_client_session_ssl). Leave unset for a SearXNG instance that does not ask for a client certificate.

#### `SEARXNG_CLIENT_KEY_FILE`

- Type: `str`
- Default: unset
- Description: Path to the private key for [`SEARXNG_CLIENT_CERT_FILE`](#searxng_client_cert_file). Omit it when the certificate file already contains the key. It has no effect unless the certificate variable is also set.

:::info

Both paths must be readable inside the container, so mount them in (for example `-v /path/to/certs:/certs:ro`) and point the variables at the mounted location. The certificate is loaded once and cached for the process, so changing the files on disk needs a restart to take effect.
...
- Type: `str`
- Default: `` (empty string)
- Description: Sets the API key for [Staan](/features/chat-conversations/web-search/providers/staan), a web search API that keeps its data in the EU.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `en-us`
- Description: Picks the region and language Staan returns results for, written as a language and country pair such as `de-de`.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `int`
- Default: `0`
- Description: How many extra passages Staan returns per result. At `0` the plain search endpoint answers with a title, a URL and one snippet. Above `0`, Staan fetches each result page, scores its passages against the query and returns up to this many chunks, which Open WebUI joins onto that result's snippet so the model reads the relevant parts of the page. Fetching pages costs more time per search than the plain endpoint.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Description: Base URL for Tavily search and extract requests. A trailing slash is stripped. Point it at a proxy or a regional endpoint if you use one.
...
- Description: Optional JSON object merged over the Linkup request defaults. Recognized keys include `depth` (`standard` or `deep`), `outputType` (`sourcedAnswer` or `searchResults`), and `url` (overrides the API endpoint). `q` (the query) and `maxResults` are injected automatically and cannot be overridden. Invalid JSON falls back to `{}`. Configurable via **Settings → Admin → Web Search** when the `linkup` engine is selected.
...
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Web Search > Jina API Base URL**.
...
#### `EXA_MAX_CONTENT_LENGTH`

- Type: `int`
- Default: `None` (the full text of every result is kept)
- Description: Caps how many characters of page text Exa returns per result, which is the lever for keeping a wide search from filling the context window. Leave it unset for no limit. A value that is not a positive integer stops the server at startup rather than being ignored, so a typo here is loud instead of silent. The same setting sits in **Settings > Admin > Web Search** and appears once the engine is Exa.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Options:
  ...
  - `microsoft_web_iq`: Uses the Microsoft Web IQ fetch API, configured by [`MICROSOFT_WEB_IQ_API_BASE_URL`](#microsoft_web_iq_api_base_url), [`MICROSOFT_WEB_IQ_API_KEY`](#microsoft_web_iq_api_key) and [`MICROSOFT_WEB_IQ_LANGUAGE`](#microsoft_web_iq_language).
...
1. If `PLAYWRIGHT_WS_URL` is not set, Playwright with Chromium dependencies will be automatically installed in the Open WebUI container on launch.
2. If `PLAYWRIGHT_WS_URL` is set, Open WebUI will connect to a remote browser instance instead of installing dependencies locally.
...
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Web Search > Firecrawl Timeout**.
...
- Type: `str`
- Default: unset (requests come from the server's own address)
- Description: Proxy used for the requests that fetch a YouTube video's transcript, in the form `https://user:password@host:port`. The same URL is used for HTTP and HTTPS. YouTube blocks transcript requests from many data center and cloud provider addresses; when it does, attaching a YouTube video fails with a message saying YouTube blocked the request and naming this setting.
- Persistence: This environment variable is a `ConfigVar` variable. It can be configured in the **Settings > Admin > Web Search > Youtube Proxy URL**.
...
- Type: `str`
- Default: Empty string. Set the model your endpoint expects, for example `whisper-1` for the OpenAI API
...
- Type: `bool`
- Default: `True`
- Description: Master switch for the voice-mode system prompt. When `True`, voice-mode chats prepend either the custom `VOICE_MODE_PROMPT_TEMPLATE` (if set) or `DEFAULT_VOICE_MODE_PROMPT_TEMPLATE`. When `False`, no voice-specific system prompt is injected: the model uses only the regular system prompt and chat history. Configurable in **Settings → Admin → Interface → Voice Mode Custom Prompt** (toggle).
...
- Type: `str`
- Options:
  - `openai`: Uses the OpenAI images API (`gpt-image-2`, `gpt-image-1`, DALL·E) for image generation.
...
- Type: `str`
- Default: ``
- Description: Default model to use for image generation (e.g., `gpt-image-2`, `gemini-3.1-flash-image`).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Options:
  - `openai`: Uses the OpenAI images API (`gpt-image-2`, `gpt-image-1`, DALL·E) for image editing.
...
- Type: `str`
- Default: ``
- Description: Specifies the model to use for image editing operations within the selected engine (e.g., `gpt-image-2`, `gemini-3.1-flash-image`).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str` (JSON)
- Default: `{}`
- Description: The environment variable is `IMAGES_OPENAI_PARAMS`. `IMAGES_OPENAI_API_PARAMS` is the key the admin config API uses for the same setting. Additional parameters for OpenAI image generation API in JSON format. Allows customization of API-specific settings such as quality parameters for DALL-E models (e.g., `{"quality": "hd"}` for dall-e-3).
...
- Type: `str`
- Default: Empty string
- Description: Base URL that [`IMAGES_GEMINI_API_BASE_URL`](#images_gemini_api_base_url) and [`IMAGES_EDIT_GEMINI_API_BASE_URL`](#images_edit_gemini_api_base_url) fall back to when they are unset. Nothing else reads it.

##### `GEMINI_API_KEY`

- Type: `str`
- Default: Empty string
- Description: API key that [`IMAGES_GEMINI_API_KEY`](#images_gemini_api_key) and [`IMAGES_EDIT_GEMINI_API_KEY`](#images_edit_gemini_api_key) fall back to when they are unset. Nothing else reads it.
...
- Options:
  ...
  - `generateContent`: Uses the generateContent endpoint (for Gemini 2.5 Flash Image and all newer Gemini image models).
- Default: ``
...
- Type: `bool`
- Default: `True`
- Description: Master switch for signing in through an OAuth or OIDC provider. When set to `False`, both the provider login and the callback are refused, so single sign-on can be turned off without deleting the client ID, secret and endpoint settings. The equivalent toggle sits in **Settings > Admin > Authentication**, alongside the LDAP one. Leave it enabled unless you want to suspend SSO while keeping its configuration.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables account creation when signing up via OAuth. Distinct from `ENABLE_SIGNUP`, and from [`ENABLE_OAUTH`](#enable_oauth), which turns provider sign-in off entirely rather than just new account creation.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: When enabled, an unauthenticated visitor to `/auth` is redirected straight to the OAuth provider's login page, skipping the "Continue with SSO" screen. It only fires on an unambiguously SSO-only deployment: exactly one OAuth provider configured, [`ENABLE_LOGIN_FORM`](#enable_login_form) set to `False` and no LDAP or trusted-header auth. The redirect is suppressed during onboarding, after a failed sign-in and for an existing session. Visit `/auth?form=true` to reach the local login form anyway (an escape hatch for admins).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: When [`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) is enabled, controls whether OAuth-related settings (config paths starting with `oauth.`) are also loaded from the database. When `False`, OAuth settings are always read from the environment variables instead.
...
:::info Read-only Admin Panel while this is `False`

The environment is authoritative, so the **OAuth / OIDC** section of **Settings > Admin > Authentication** is shown read-only: values stay visible, no control accepts input, and saving the page leaves them untouched. Change them in the environment and restart, or set this to `True` to manage them in the Admin Panel.

[`ENABLE_PERSISTENT_CONFIG`](#enable_persistent_config) is the one in charge. While it is `False` nothing is saved whatever this variable says, so both have to be `True` for an edit to last.
...
- Status: **Not implemented.** Open WebUI reads no such variable, so setting it has no effect. It was named in the v0.6.33 release notes, but the change that shipped reads [`ENABLE_OAUTH_EMAIL_FALLBACK`](#enable_oauth_email_fallback) instead.
- Instead: to sign users in from a provider that returns no email address, set `ENABLE_OAUTH_EMAIL_FALLBACK` to `true`, which gives the account a placeholder address built from the provider name and the `sub` claim.
...
- Type: `bool`
- Default: `False`
- Description: If enabled, replaces the local user's name with the value of [`OAUTH_USERNAME_CLAIM`](#oauth_username_claim) on every OAuth login when the two differ.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: If enabled, replaces the local user's email with the value of [`OAUTH_EMAIL_CLAIM`](#oauth_email_claim) on every OAuth login when the two differ. The update is skipped, and an error logged, when another account already uses the new address.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: When a provider does not supply an email address during an OAuth login, generate a placeholder address of the form `<provider>@<sub>.local` so the login can continue instead of being rejected. This applies only when the provider returned no email. The account is then identified by that placeholder, so the `sub` claim it is built from has to be stable, and email-based features such as account merging via `OAUTH_MERGE_ACCOUNTS_BY_EMAIL` will not behave as they would with a real address.
...
- Usage: Set to `true` to enable the token exchange endpoint at `/api/v1/oauth/{provider}/token/exchange`. When enabled, external applications can POST to this endpoint with an OAuth access token to obtain an Open WebUI session token. Set [`OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS`](#oauth_token_exchange_trusted_client_ids) alongside it: **leaving it empty performs no client check at all**, so any token your provider recognises is accepted. Neither setting is per-provider: the same trusted client list applies whichever provider the request names, so a provider without introspection support is refused entirely once the list is set.

:::danger

**This endpoint issues a session as whichever user the presented token belongs to.**

An access token is a bearer credential. Your provider's userinfo endpoint confirms that a token is valid, but it does not reveal which OAuth client the token was issued to. Unless the issuing client is checked, a token minted for any client registered with your provider is accepted, including applications you do not control. Someone who obtains an access token for one of your users, for example by having them sign in to an unrelated OAuth application, could exchange it for a session as that user, inheriting that user's role.

`OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS` is what restricts this to clients you have chosen to trust. `OAUTH_ALLOWED_DOMAINS` does not: it governs which user identities may sign in, and an impersonated user is a legitimate member of an allowed domain.

:::

**Request Example:**
```bash
curl -X POST "http://localhost:8080/api/v1/oauth/oidc/token/exchange" \
  -H "Content-Type: application/json" \
  -d '{"token": "<access token issued to a trusted client>"}'
```
...
- Type: `int`
- Default: *(unset, no limit)*
- Description: Caps how many token-exchange attempts are accepted per window. Unset, or the literal `none`, leaves the endpoint unthrottled. Set an integer to enable throttling, for example `15`. This is a best-effort brake only: the caller has no established identity before the provider validates the token, so requests are bucketed by client address, which is unreliable behind a proxy that lets clients influence the forwarded address. Treat it as noise reduction, not as the control that keeps the endpoint safe. That remains `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS`.

#### `OAUTH_TOKEN_EXCHANGE_RATE_LIMIT_WINDOW`

- Type: `int`
- Default: `180`
- Description: Length of the token-exchange rate-limit window in seconds. Only has an effect when `OAUTH_TOKEN_EXCHANGE_RATE_LIMIT` is set.

**Requirements:**
- The user must already exist in Open WebUI (they must have signed in via the web interface at least once)
- The OAuth provider must be properly configured, and must implement RFC 7662 token introspection if you restrict the issuing client
- `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS` must name the OAuth client the token was issued to
...
- Default: empty
- Description: The OAuth client IDs whose tokens the token exchange endpoint will accept. When set, Open WebUI resolves which client a presented token was issued to using RFC 7662 token introspection, and rejects any token minted for a client that is not listed, or whose issuing client it cannot determine.
- Usage: `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS=internal-cli,reporting-service`

:::danger

**An empty value means the issuing client is not checked at all.** The exchange then accepts any token the provider's userinfo endpoint validates, whichever application obtained it, so any app your users sign into with the same provider can trade its token for an Open WebUI session as that user. Set this whenever the endpoint is enabled.

Only the `client_id` reported by token introspection is honoured. The `aud` field is ignored, because it names intended resource servers rather than the issuing client, and some identity providers allow any client to place another client's identifier there.

This is a deploy-time environment variable and cannot be changed from the admin interface, so a compromised administrator session cannot widen the trust boundary at runtime.

:::

:::info

**Provider support.** Resolving the issuing client requires RFC 7662 token introspection. Google, Microsoft Entra ID, GitHub and Feishu do not implement it, so the client restriction cannot be used with those providers: every exchange is refused while it is set, and the reason is recorded in the server log. Providers that do implement it include Keycloak, Authentik, Okta and Zitadel.
...
- Type: `str`
- Description: Defines the trusted request header that determines the user's role (`admin`, `user`, or `pending`) when using trusted header authentication. When set, the user's role is updated to match the header value on every sign-in. Invalid values are ignored with a warning. A role change from this header ends that account's live WebSocket connections, which reconnect and re-authenticate on their own, so the previous role cannot linger in an open tab. See [SSO docs](/features/authentication-access/auth/sso) and [When a Role Change Takes Effect](/features/authentication-access/rbac/roles#when-a-role-change-takes-effect).
...
- Default: Empty string (' '), since `None` is set as default.
- Description: Specifies the code challenge method for OAuth authentication. Set to `S256` when PKCE is required by the provider. This now applies to **every** sign-in provider, Google, Microsoft and GitHub as well as generic OpenID Connect, where it previously took effect on the OIDC provider alone. Any value other than `S256` is rejected at startup.
- Persistence: This environment variable is a `ConfigVar` variable.

#### `OAUTH_CLIENT_TIMEOUT`

- Type: `int`
- Default: Empty string (the HTTP client's own default)
- Description: Timeout in seconds for the HTTP client the OAuth login flow uses to reach the provider's token and userinfo endpoints.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `groups`
- Description: Specifies the group claim for OAuth authentication. `OAUTH_GROUPS_CLAIM` is read as well and wins when both are set. The admin UI field and the persisted key keep the singular name.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: When enabled, groups from OAuth claims that don't exist in Open WebUI will be automatically created. Groups matching [`OAUTH_BLOCKED_GROUPS`](#oauth_blocked_groups) are never created.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `[]`
- Description: JSON array of OAuth group names that group management leaves alone. At sign-in, a matching group is never created (with `ENABLE_OAUTH_GROUP_CREATION`), and users are never added to or removed from it (with `ENABLE_OAUTH_GROUP_MANAGEMENT`). Each entry matches exactly, as a shell-style wildcard (`*`, `?`) or as a regular expression. To deny sign-in by role, use [`OAUTH_ALLOWED_ROLES`](#oauth_allowed_roles).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: If enabled, adds `client_id` to the token request that exchanges the authorization code. Some providers reject the exchange without it.
...
- Type: `bool`
- Default: `False`
- Description: Enables the group management feature. Also configurable in the LDAP section of **Settings > Admin > Authentication**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: If a group from LDAP does not exist in Open WebUI, it will be created automatically. Also configurable in the LDAP section of **Settings > Admin > Authentication**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `str`
- Default: `memberOf`
- Description: Specifies the LDAP attribute that contains the user's group memberships. `memberOf` is a standard attribute for this purpose in Active Directory environments. Also configurable in the LDAP section of **Settings > Admin > Authentication**, where group management cannot be saved while this is left empty.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables the SCIM 2.0 endpoints at `/api/v1/scim/v2`, so an identity provider can create, update and deactivate users and groups in Open WebUI automatically. Included in the free edition with no license key and no user limit. Also accepts the older name `SCIM_ENABLED`.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `str`
- Default: Empty string
- Description: Bearer token your identity provider must present on SCIM requests. Treat it as a credential with user and group management rights, and set it whenever SCIM is enabled.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `str`
- Default: Empty string
- Description: Name of the OAuth provider that SCIM-provisioned identities correspond to, for example `microsoft` or `oidc`, which lets Open WebUI store the provider's `externalId` against each user. Leaving it unset while SCIM is enabled logs a warning at startup and skips `externalId` storage, so provisioned accounts may not line up with the same users signing in through SSO.
- Persistence: Set via environment variable; applied at startup (not a `ConfigVar`).
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to attach web pages in chats. When `False`, the **Attach Webpage** option stays in the attach menu for non-admins but is greyed out with a tooltip, and an attach sent anyway is refused. Configurable per-group as **Allow Web Upload**.

#### `USER_PERMISSIONS_CHAT_DELETE`

- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to delete chats. When `False`, the delete controls are hidden from non-admins rather than offered and refused, in the sidebar, the chat menu, search, archived chats and data controls, and the **Delete chat** shortcut stops doing anything. Archiving is unaffected. Deleting a folder that still holds chats needs the same permission and is refused rather than hidden. Configurable per-group as **Allow Chat Delete**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to export chats. When disabled, non-admins cannot download a conversation as a file. Also configurable per-group in **Admin Panel > Users > Groups > Permissions > Chat Permissions > Allow Chat Export**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to import chats, and to fork one, which copies an existing conversation into a new one. When `False`, non-admins cannot import, and the **Fork chat** action and the `/fork` command are hidden rather than shown and refused. Configurable per-group as **Allow Chat Import**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Enables or disables user permission to use the [memory feature](/features/chat-conversations/memory). When `False`, the affected users lose the **Personalization** settings tab, the memory endpoints answer them with `403`, the model is not given the memory tools and their stored memories no longer reach the system context. Admins are exempt.
- Persistence: This environment variable is a `ConfigVar` variable.
...
These settings control whether the **Access Control** panel shows its access list, where users add specific people and groups to an item. Which grants the server accepts is set by [`USER_PERMISSIONS_ACCESS_GRANTS_ALLOW_USERS`](#user_permissions_access_grants_allow_users), [`USER_PERMISSIONS_ACCESS_GRANTS_ALLOW_GROUPS`](#user_permissions_access_grants_allow_groups) and the public sharing settings below.

#### `USER_PERMISSIONS_WORKSPACE_MODELS_ALLOW_SHARING`

- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of workspace models.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of workspace knowledge.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of workspace prompts.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of workspace tools.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of workspace skills.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Access Control** panel of notes. Parent of [`USER_PERMISSIONS_NOTES_ALLOW_PUBLIC_SHARING`](#user_permissions_notes_allow_public_sharing).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Shows the access list in the **Share** dialog of chat folders, where users add specific users or groups.
- Persistence: This environment variable is a `ConfigVar` variable.

### Sharing (Public)

These settings control whether users can share workspace items **publicly**. All default to `False`, and each has a parent permission that must be on before its toggle appears: the matching private-sharing setting above for workspace items and notes, `USER_PERMISSIONS_CHAT_SHARE` for chats and `USER_PERMISSIONS_FEATURES_CALENDAR` for calendars.

:::warning Re-check tools and notes after an upgrade

On instances upgraded from a release predating these two variables, the admin panel showed **Tools Public Sharing** and **Notes Public Sharing** as on while the action was still refused, and saving any permission on that page wrote both as enabled for real. A value already saved is not rolled back, so open **Admin Panel > Users > Groups**, edit **Default permissions** and confirm both read the way you intend.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of workspace tools. Requires `USER_PERMISSIONS_WORKSPACE_TOOLS_ALLOW_SHARING`; with that off, neither the **Public** option nor this toggle is shown. When disabled, public grants are stripped from tool create and update payloads, leaving per-user and per-group grants alone. Admins are exempt. Configurable per-group as **Tools Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of notes. Requires `USER_PERMISSIONS_NOTES_ALLOW_SHARING`; with that off, neither the **Public** option nor this toggle is shown. When disabled, public grants are stripped from note create and update payloads, leaving per-user and per-group grants alone. Admins are exempt. Configurable per-group as **Notes Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Enables or disables **public sharing** of chats. Public means every signed-in user of the instance rather than the open internet; for links that work without an account see [`USER_PERMISSIONS_CHAT_ALLOW_OPEN_SHARING`](#user_permissions_chat_allow_open_sharing). When disabled, non-admins lose the **Public** option in the share modal but can still share with named users and groups. Requires `USER_PERMISSIONS_CHAT_SHARE`. Admins are exempt. Configurable per-group as **Chats Public Sharing**.
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `False`
- Description: Allows non-admin users to mark a chat share link **Open**, readable by anyone holding the link with no account and no sign-in. This is the only sharing setting that exposes content beyond the instance's own users, so leave it disabled unless publishing conversations to the internet is intended. It grants read access only, and only chats accept this kind of grant. When disabled, the **Open** option is hidden and an open grant sent to the API is stripped before saving. Requires `USER_PERMISSIONS_CHAT_SHARE`. Admins are exempt. Configurable per-group as **Chats Open Sharing**. See [Open links](/features/chat-conversations/chat-features/chatshare#open-links-no-sign-in).
- Persistence: This environment variable is a `ConfigVar` variable.
...
- Type: `bool`
- Default: `True`
- Description: Controls whether database migrations are automatically run on startup. In multi-pod or multi-worker deployments, set this to `False` on all pods except one to designate a "master" pod responsible for migrations, preventing race conditions or schema corruption. It decides **which** instance migrates and nothing else: it does not make a mixed-version fleet safe, so a release that changes the schema still has to reach every pod at once rather than one at a time. See [Updates and Migrations](/troubleshooting/multi-replica#updates-and-migrations).
...
- Description: Specifies the database type (e.g., `sqlite`, `postgresql`). This is used in conjunction with other individual parameters to construct the `DATABASE_URL` if a complete `DATABASE_URL` is not explicitly defined.
- Persistence: No
...
- Type: `float`
- Default: `60`
- Description: Minimum interval, in seconds, between writes of a user's `last_active_at` timestamp to the database (this timestamp drives online/"active" presence, evaluated over a ~3-minute window). The write is throttled **per user**: within the interval, repeat activity does not hit the database. Set it to `0` to disable throttling and write on every request.

:::warning Raise this at scale, and never set it to zero there
Throttling is on by default at 60 seconds, which already collapses thousands of writes into at most one per user per interval. **Setting it to `0` disables throttling, so essentially every authenticated request issues its own `UPDATE users SET last_active_at = ... ` plus a `COMMIT`.** On a busy instance that is a continuous flood of tiny write transactions which amplifies database load and consumes connection-pool capacity for no functional benefit. Presence only needs about minute granularity.

**Keep it below 180 seconds.** Active presence counts users whose timestamp falls in the last 180 seconds, and that window is fixed in the code. An interval at or above it means a user's timestamp can age past the window before the next write is allowed, so they drop out of the count and reappear on the next write, which shows up as a sawtooth in the active-user metric rather than a steady figure.

- **Recommended:** `120`. That halves the presence writes again while leaving a full minute of headroom inside the 180 second window, so a user who is still active is always counted. The default of `60` is also fine and needs no change.
- Above `180` the count stops being reliable. Only go there if you do not use the active-user figure or the metric that reports it.
...
SQLCipher is not supported since v0.9.0. The async database layer introduced in that release has no SQLCipher driver, so Open WebUI refuses to start when `DATABASE_TYPE=sqlite+sqlcipher` or a `sqlite+sqlcipher://` URL is set. For encryption at rest, use an encrypted filesystem or PostgreSQL with encrypted storage. To keep the data of an existing SQLCipher database, decrypt it first as described in [Upgrading an Existing SQLCipher Database](/reference/database-schema#upgrading-an-existing-sqlcipher-database).
...
- Type: `str`
- Description: Specifies the URL of the Redis instance or cluster host for storing application state. A Redis-protocol server such as [Valkey](/tutorials/integrations/valkey) or Dragonfly should work in its place, on the same `redis://` and `rediss://` schemes, though neither is officially supported yet; use a recent release of whichever you pick.
- Examples:
...
- Type: `int` (seconds)
- Default: `3600`
- Description: A safety net on the entries Redis holds while a response is streaming, which let a reconnecting browser pick a reply back up where it left off. An entry is deleted as soon as its response finishes, so this timeout is not the normal lifetime of one. It catches the entries whose cleanup never ran, a worker killed mid-stream or a process that died, which previously stayed in Redis forever and accumulated. Lower it to reclaim that memory sooner, raise it if a long-running response could legitimately outlive the default hour, or set `0` to keep orphaned entries indefinitely as before. Applies only when Redis is in use.
- Persistence: No
...
- Type: `int` (seconds)
- Default: `300`
- Description: How long a background task's Redis entry survives without a heartbeat from the worker that owns it. Each worker refreshes its own tasks at a quarter of this interval, so a task is cleaned up only when its worker has gone silent past the full timeout, a crash among them; this stops tasks from a dead worker blocking a stop request forever on multi-worker deployments. The listing of active tasks keeps entries whose key has already expired. Set `0` to keep task entries indefinitely as before, which also skips the heartbeat. Values below `60` fall back to the default. Applies only when Redis is in use.
- Persistence: No
...
- Type: `float` (seconds) or empty string for None
- Default: None (no timeout, uses redis-py library default)
- Description: Sets the read/write timeout on all Redis client connections, the companion to [`REDIS_SOCKET_CONNECT_TIMEOUT`](#redis_socket_connect_timeout), which only bounds the initial connect. It applies to plain, Cluster and Sentinel modes, sync and async alike.

:::warning Not for the subscriber connections

Leave this unset unless you have a reason. A timeout here also lands on connections that sit idle by design, above all the pub/sub subscriber that carries stop requests and configuration changes between instances: it waits quietly for messages, and a socket timeout ends that wait as an error. If you do set it, keep it comfortably longer than the quiet periods you expect, and check that stopping a reply from another instance still works.
...
- Type: `bool`
- Default: `True`
- Description: Takes effect when `WEBSOCKET_MANAGER=redis`. Live updates addressed to a specific user, channel or note are published on a Redis channel of their own, and each instance skips updates for rooms none of its connected clients have joined. This saves the CPU each instance would spend decoding every update, which adds up on multi-instance deployments. Acknowledgements and control messages stay on the shared channel. Set to `False` to send everything through the one shared channel.

:::warning

**Every Instance Must Run the Same Mode**

Changing this setting requires stopping or updating all instances together.
...
- Type: `int`
- Default: unset, meaning no timeout
- Description: How long the server waits for someone to answer an interactive prompt, form or dropdown raised by a tool or action. Left unset it waits indefinitely, so a prompt nobody answers holds that call open. On timeout the call returns an `error` key to the calling plugin rather than raising, so check the returned value instead of wrapping it in try/except. A value that is not a whole number falls back to `300`. The browser session is untouched, so a tab that is simply unattended stays connected and can be prompted again. See [Interactive Events](/features/extensibility/plugin/development/events#interactive-events).
...
Both options only settle **which** process runs the migration. Neither makes it safe to leave instances on the old version while the new one migrates, so roll a release that changes the schema out to every instance at once. See [Updates and Migrations](/troubleshooting/multi-replica#updates-and-migrations).
