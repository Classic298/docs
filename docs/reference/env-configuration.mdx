...
It bounds **both** paths Open WebUI offloads on: the AnyIO limiter behind `run_in_threadpool`, and the event loop's default executor behind `asyncio.to_thread`. The second one previously had no setting at all and sat at a fixed `min(32, cpu_count + 4)`, so vector database calls, storage reads, LDAP binds and password verification shared at most 32 slots however high this was set; because `cpu_count()` reports the host's cores rather than a container's limit, that figure was also derived from the whole machine rather than the container. The two pools are independent, so the per-process thread ceiling is twice this value, and both still fill lazily.
