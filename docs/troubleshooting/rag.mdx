...
| **RAG System Context** | **On** (`RAG_SYSTEM_CONTEXT=True`) | Keeps KV cache stable across turns |
...
| **RAG System Context** | **On** (`RAG_SYSTEM_CONTEXT=True`) | Enables prompt caching (OpenAI, Anthropic, Gemini all benefit) |
...
| **Top K** | `10` | Moderate — can be overridden per-chat via Chat Controls |
| **RAG System Context** | **On** | Benefits both local and cloud |

:::note Per-chat overrides
You can override Top K and Full Context Mode on a per-chat basis in the **Chat Controls** (right sidebar). This lets you keep conservative global defaults while going aggressive for specific conversations with large-context models.
:::
