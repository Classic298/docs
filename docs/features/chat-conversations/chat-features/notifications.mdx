...
| Requirement | Where |
|---|---|
| [`ENABLE_USER_WEBHOOKS`](/reference/env-configuration#enable_user_webhooks) enabled (default: off) | **Settings > Admin > General**, the **User Webhooks** toggle |
...
Five events can be subscribed to:
...
| `retrieval.content.process_failed` | A file fails to be processed for retrieval. |

The retrieval failure notification names the file and carries the processing error, so you can tell which upload failed and why without opening the knowledge base.
...
Earlier versions had a single **Notification Webhook** URL under **Settings > Account** that fired on chat responses. That field is replaced by targets, and an existing URL is **migrated automatically** into a target subscribed to all five events, so notifications keep arriving without any action.
