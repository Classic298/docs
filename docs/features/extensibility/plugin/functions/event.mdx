...
- **Provider-driven change tracking.** An SSO login is a write path, not just a read. With `ENABLE_OAUTH_GROUP_MANAGEMENT` it emits `group.member_added` and `group.member_removed` per membership transition, plus `group.created` for anything `ENABLE_OAUTH_GROUP_CREATION` makes; a login that refreshes the profile emits `user.updated`, naming the changed keys in `updated_fields`. All of them carry `source: 'oauth'`, so membership and profile changes an identity provider drove land in the same sink as the ones an admin made.
