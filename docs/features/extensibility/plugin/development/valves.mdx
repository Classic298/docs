...
Hence, Valves and UserValves class can be defined in a `Pipe`, `Filter`, `Action` or `Tools` class. An `Event` class can carry `Valves` too, but never `UserValves`, since event handlers run without a user.

Function Valves are configurable by admins alone, since Functions are admin-only. Tool Valves can also be edited by the tool's owner and by anyone holding a write grant on it, so treat a tool's valves as readable by everyone you share write access with. UserValves are configurable by any user directly from a chat session.
...
# Define Valves and UserValves
class Filter:
    # Notice the indentation: Valves and UserValves are declared as
    ...
    def inlet(self, body: dict, __user__: dict):
        ...
        # But this raises TypeError, a pydantic model is not subscriptable:
...
Valve titles, help text and dropdown option labels can be stored per language, from the globe selector in the tool or function editor. Only the labels change, the field names and the values you receive in code stay exactly as written. See [Translations](/features/administration/translations#translating-valve-labels).
...
Password input only masks values **in the UI**, it does not change how they are stored. By default, Valve values (including secrets) are saved as plaintext JSON in the database. To encrypt them at rest, an administrator can set [`ENABLE_VALVE_ENCRYPTION=true`](/reference/env-configuration#enable_valve_encryption), which encrypts Tool and Function Valve values with Fernet (AES-128) using a key derived from [`WEBUI_SECRET_KEY`](/reference/env-configuration#webui_secret_key).

:::warning
A `WEBUI_SECRET_KEY` of any length is hashed into a valid Fernet key, with one exception: a key exactly 44 characters long is used verbatim and must therefore be url-safe base64 that decodes to 32 bytes. What matters most is that it stays stable, since rotating it makes previously encrypted Valve values unreadable and they have to be re-entered. See [`ENABLE_VALVE_ENCRYPTION`](/reference/env-configuration#enable_valve_encryption) for details.
:::
...
When a field should hold **several** of the offered options rather than one, declare it as a list and use the multiselect input type. It renders a dropdown of checkboxes, with the chosen labels shown in the closed field:
...
Options are declared exactly as for a select: a plain list of strings, or `{"value": ..., "label": ...}` pairs, or the name of a method that returns them.

The value stays a real list end to end. Other `list` valves are edited as comma-separated text and split back apart on save, which makes any option containing a comma impossible to express; a multiselect field skips that round trip entirely, so your code receives the selected values as a list exactly as they were defined.
