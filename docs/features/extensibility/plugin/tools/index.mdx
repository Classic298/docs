...
![Installed tools in the workspace](/images/workspace/tools-list.png)
...
The globe selector in the tool editor stores a name, a description and valve labels per language. User valve labels are the ones worth translating first, since those are what ordinary users see in Chat Controls. See [Translations](/features/administration/translations).
...
1.  **Every model at once (global default, fastest)**:
    *   Navigate to **Settings → Admin → Models**.
...
2.  **Per-Model Override**: edit a specific model in **Settings → Admin → Models** and set **Function Calling** to `Legacy` under **Model Params → Advanced Params**. Overrides the global default for that model only.
...
2.  **Per-Model Override**:
    *   In **Settings → Admin → Models**, pick a specific model and click its edit button.
...
:::tip Minimum model tier (not a maintained list)
...
By default a model runs its tool calls as soon as it makes them. An administrator can offer a second mode by turning on **Tool Permissions** in **Settings > Admin > Interface** ([`ENABLE_TOOL_PERMISSIONS`](/reference/env-configuration#enable_tool_permissions)). The feature is **experimental** and off by default.

Once it is on, the chat input's ➕ menu gains a **Tool Permissions** entry with two choices:

| Mode | Behaviour |
| :--- | :--- |
| **Full access** (default) | Tool calls run immediately, as they always have. |
| **Ask for approval** | The reply stops at the first tool call. The pending call shows **Allow** and **Deny** in the message, and the model only continues once you answer. If the model asked for several tools at once you approve them one at a time; the rest wait their turn. |

You can answer from the keyboard as well: **Allow tool call** (`Cmd+Alt+Enter`) and **Deny tool call** (`Cmd+Alt+Backspace`), both rebindable under [keyboard shortcuts](/features/chat-conversations/chat-features/keyboard-shortcuts).

Your choice is saved to your user settings, so it carries over to new chats until you change it back. A denied call is reported back to the model as an error, so it can react rather than silently losing the result, and the tool call is marked as denied in the message. Pressing **Stop** on a reply that is waiting for approval finalizes it, denying whatever is still pending.

Two cases always run with full access regardless of the setting: **automations**, which run on a schedule with nobody watching, and **channel** replies.

:::info Requires a saved chat
Approval works by persisting the pending call on the assistant message, so a temporary chat has nowhere to store it. Approval prompts also survive a page reload, since the pending state lives in the message rather than in the browser.
:::
...
| **Files** | *Requires per-model "Files" category enabled, the model's **File Upload** capability on with **File Context** off, the user's `chat.file_upload` permission, and at least one file attached to the current chat. Covers files the user attached to the conversation, not knowledge bases. Turning **File Context** off for this is also the setup that keeps the prompt prefix cacheable, so turn **Citations** off alongside it and put your citation rules in the system prompt instead; see [prompt caching](/features/chat-conversations/prompt-caching).* |
| `list_chat_files` | List the files attached to the current chat, with each file's ID, filename, content type and size. |
| `query_chat_files` | Semantic / RAG search across the attached files, or one file when given its ID. Results are returned as cited chunks. |
| `grep_chat_files` | Exact text / regex search across the attached files, returning matching lines with file IDs and line numbers, or just per-file counts. Capped at 50 matches by default, the same [`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches) limit the knowledge greps use. |
| `view_file` | Read one of the attached files by ID, with character pagination or a line range. Same tool, and the same [`VIEW_FILE_DEFAULT_MAX_CHARS`](/reference/env-configuration#view_file_default_max_chars) / [`VIEW_FILE_MAX_CHARS`](/reference/env-configuration#view_file_max_chars) sizing, as when it reads a knowledge file. |
...
| `list_memory_paths` | Lists the paths memories are filed under. |
| `read_memory_path` | Reads the memories stored under one path. |
| `list_memories` | Lists all stored memories for the user. |
| `update_memory` | Applies a batch of `add`, `replace`, `move` or `remove` operations. |
...
| `search_chats` | Text search across chat titles and message content, skipping the chat the model is currently in. A multi-word query matches chats containing all of the words in any order, with exact phrase matches ranked first. Returns matching chat IDs and snippets. |
...
| **Task Management** | *Requires per-model "Task Management" category enabled (default: on) AND a saved chat, since the checklist is stored on the chat. Not available in a [temporary chat](/features/chat-conversations/chat-features/url-params).* |
...
| `create_automation` | Create a scheduled automation with a name, prompt, and RRULE schedule, optionally filed into a folder. Uses the current chat model. |
| `update_automation` | Update an existing automation's name, prompt, schedule, folder, or model. |
...
| **Skills** | *Requires at least one skill to be attached to the model via **Workspace or Settings > Admin > Models → Edit → Skills**. The model receives a summary of attached skills in its system prompt and can call `view_skill` to load full instructions on demand. No separate builtin tools category checkbox is needed; attaching a skill is the only requirement.* |
...
| **Ask User** | *Requires per-model "Ask User" category enabled (default: on) AND a saved chat, since the pending question is stored on the assistant message. Not available in a [temporary chat](/features/chat-conversations/chat-features/url-params).* |
| `ask_user` | Pause the reply and put a question card above the message box, then continue once the user answers. It must be the only tool call in that turn, and only one `ask_user` per turn; anything else comes back to the model as an error. The pending question survives a page reload. |
...
- knowledge attached to the [folder](/features/chat-conversations/chat-features/conversation-organization) the chat sits in,
...
| `fetch_url` | `url` (required) | Plain text content, untruncated unless the admin sets `WEB_FETCH_MAX_CONTENT_LENGTH` |
| **Knowledge Base** | | |
| `list_knowledge` | `knowledge_id` (optional, pages into one base's files), `skip` (default: 0), `count` (default: 50, capped at 200) | `{knowledge_bases: [{id, name, description, file_count, files: [{id, filename}]}], files: [{id, filename, updated_at}], notes: [{id, title}]}`, and with `knowledge_id` set the base also carries `files_skip`, `files_count`, `files_total` and `has_more` |
...
| `grep_knowledge_files` | `pattern` (required; regex auto-detected), `file_id` (optional, single-file mode), `case_insensitive` (default: false), `count_only` (default: false) | Matching lines with file IDs, filenames, and 1-indexed line numbers (capped at 50 matches by default, see [`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches)) |
| `view_file` | `file_id` (required), `offset` (default: 0), `max_chars` (default: 10000, cap: 100000; both configurable), `line_numbers` (default: false), `start_line` / `end_line` (optional; line-based addressing overrides `offset`/`max_chars`) | `{id, filename, content, updated_at, created_at}`, includes `truncated`, `total_chars`, `next_offset` when paginated, or `total_lines`, `showing_lines`, `next_start_line` in line mode |
| `view_knowledge_file` | `file_id` (required), `offset` (default: 0), `max_chars` (default: 10000, cap: 100000; both configurable) | `{id, filename, content, knowledge_id, knowledge_name}`, includes pagination metadata when truncated |
...
| **Memory** | | |
| `search_memories` | `query` (default: `""`), `count` (default: 5), `type` (default: `all`), `path`, `memory_id` | Array of `{id, type, path, content, created_at, updated_at}` |
| `list_memory_paths` | `query` (default: `""`), `count` (default: 100), `type` (default: `all`) | The paths memories are filed under |
| `read_memory_path` | `path` (required), `count` (default: 50), `type` (default: `all`), `include_children` (default: `true`) | The memories stored under that path |
| `update_memory` | `operations` (required, a list of `add`, `replace`, `move` or `remove` entries) | Per-operation results |
| `add_memory` | `content` (required), `type` (default: `user`), `path` | `{status: "success", id, type, path}` |
| `replace_memory_content` | `memory_id` (required), `content` (required), `type`, `path` | `{status: "success", id, type, path, content}` |
| `delete_memory` | `memory_id` (required) | `{status: "success", message}` |
| `list_memories` | None | Array of `{id, type, path, content, created_at, updated_at}` |
| **Notes** | | |
...
| `write_note` | `title` (required), `content` (required; markdown text. A JSON object or array is stored as a fenced `json` code block rather than left in the note as structured data) | `{status: "success", id}` |
| `replace_note_content` | `note_id` (required), `content` (optional; the new markdown for a whole-note update. A JSON object or array is stored as a fenced `json` code block), `operations` (optional; range edits, each `{"action": "replace", "content": ...}` or `{"action": "replace_range", "start", "end", "content", "expected"}`), `title` (optional) | `{status: "success", id, title, updated_at, applied_operation_count}`. Errors carry a machine-readable `code` (`not_found`, `write_access_denied`, `invalid_range`, `range_out_of_bounds`, `expected_mismatch`, `overlapping_operations`, `content_required`, …) |
| **Chat History** | | |
| `search_chats` | `query` (required; an exact phrase or a set of keywords), `count` (default: 5), `start_timestamp`, `end_timestamp` | Array of `{id, title, snippet, updated_at}`. `snippet` is `Title match: <title>` when the query was found in the title but not in any message |
...
| **Automations** | | |
| `create_automation` | `name` (required), `prompt` (required), `rrule` (required), `folder_id` (optional, files the generated chats into one of the user's folders) | `{status, id, name, model_id, folder_id, is_active, next_runs}` |
| `update_automation` | `automation_id` (required), `name`, `prompt`, `rrule`, `model_id`, `folder_id` (all optional; only provided fields are changed). A blank `model_id` or `folder_id` keeps the current value; `folder_id: null` clears the folder | `{status, id, name, model_id, folder_id, is_active, next_runs}` |
| `list_automations` | `status` (optional: `"active"`, `"paused"`, or omit for all), `folder_id` (optional; one of the user's own folders, an empty string or omitting it lists across all folders), `count` (default: 10) | `{automations: [{id, name, folder_id, prompt_snippet, model_id, rrule, is_active, last_run_at, next_runs}], total}` |
...
| **Ask User** | | |
| `ask_user` | `questions` (required, 1-3 objects of `{id, header, question, options}` where `options` is 2-3 `{label, description}` pairs and an optional per-question `allow_other`), `allow_other` (default: `true`), `timeout_ms` (default: `120000`, clamped to 60000-240000) | `{status: "answered", answers}` keyed by question id, or `{status: "cancelled", answers: {}}` when the card times out. Dismissing the card returns a rejection error instead |
| **Sub-agents** | | |
| `delegate_task` | `task` (required), `context` (optional; decisions, findings or file paths to carry across), `file_ids` (optional list; attachments from the parent chat to hand to the sub-agent, which otherwise receives no files at all. An ID that is not attached to the parent chat fails the call before anything runs), `background` (default: false; the parameter is removed from the schema entirely unless [`SUBAGENTS_BACKGROUND_ENABLED`](/reference/env-configuration#subagents_background_enabled) is on) | Foreground: the sub-agent's final answer as text, truncated at [`SUBAGENTS_MAX_OUTPUT`](/reference/env-configuration#subagents_max_output). Background: a JSON handle `{status: "dispatched", delegation_id, subagent_chat_id, mode, task}`, with the result posted into the parent chat later. Errors are returned as an `Error: ...` string rather than raised |
...
| **Ask User** | `ask_user` | Pause a response to ask the user a clarifying question |
| **Memory** | `search_memories`, `list_memory_paths`, `read_memory_path`, `list_memories`, `update_memory`, `add_memory`, `replace_memory_content`, `delete_memory` | Search and manage user memories |
...
`features.memories` reaches beyond the tool list. It also gates the [memory system context](/features/chat-conversations/memory#memory-system-context), so a user without the permission gets no stored memories injected into their prompt either, whatever the chat request asks for.
:::
...
For the best out-of-the-box agentic experience, administrators can enable **Web Search**, **Image Generation**, and **Code Interpreter** as default features for a model. In **Settings > Admin > Models**, click the pencil (**Edit**) on your target model and toggle these three on under **Default Features**. This ensures they are active in every new chat by default, so users get the full tool-calling experience without manually enabling each toggle. Users can still turn them off per-chat if needed.
:::
