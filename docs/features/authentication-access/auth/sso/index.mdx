...
| `ENABLE_OAUTH_PERSISTENT_CONFIG`      | `false`            | Load OAuth settings from the database (when persistent config is on). Default `false` keeps environment variables authoritative for OAuth and shows the OAuth section of the Admin Panel read-only; set `true` to manage OAuth in the Admin Panel. |
| `ENABLE_OAUTH`                        | `true`             | Master switch for provider sign-in. Set `false` to turn SSO off while keeping the client ID, secret and endpoints configured. Also a toggle in **Settings > Admin > Authentication**, next to the LDAP one. |
...
| `ENABLE_OAUTH_TOKEN_EXCHANGE`         | `false`            | Enables the token exchange endpoint for external apps to exchange OAuth tokens for Open WebUI JWTs. Pair it with `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS`. |
| `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS` | *empty*        | Comma-separated OAuth client IDs whose tokens the exchange endpoint accepts. Empty performs no client check, accepting any token the provider validates. |
...
2. **Persistent Config Behavior**: When `ENABLE_OAUTH_PERSISTENT_CONFIG=true`, OAuth settings are stored in the database after first launch. To change environment variables after initial setup, either:
   ...
   With the default `ENABLE_OAUTH_PERSISTENT_CONFIG=false`, the **OAuth / OIDC** section of **Settings > Admin > Authentication** is read-only, so the Admin Panel is not an option until you set the variable to `true`. See [Read-only OAuth settings in the Admin Panel](#read-only-oauth-settings-in-the-admin-panel).
...
### Read-only OAuth settings in the Admin Panel

`ENABLE_OAUTH_PERSISTENT_CONFIG` decides where the OAuth settings live, and the Admin Panel follows it. With the default `false`, they are read from your environment variables on every start, so the **OAuth / OIDC** section of **Settings > Admin > Authentication** is shown read-only:

- All values remain visible, and the section is dimmed with every field and switch inert, including the **OAuth / OIDC** master toggle.
- A note above the section explains that the settings are read from environment variables and names `ENABLE_OAUTH_PERSISTENT_CONFIG`.
- Saving the Authentication page leaves the OAuth settings untouched. Other sections on that page, such as LDAP, save as usual.

This replaces the older behavior, where the fields were editable, a save appeared to succeed and the values reverted to the environment on the next restart.

Set `ENABLE_OAUTH_PERSISTENT_CONFIG=true` to make the section editable and have saved values persist in the database. To keep the environment authoritative, leave it at `false` and change your environment variables instead, then restart.

`ENABLE_PERSISTENT_CONFIG` is the one in charge. When it is `false`, nothing is saved to the database whatever `ENABLE_OAUTH_PERSISTENT_CONFIG` says, so the OAuth variable then only decides whether the section can be typed into, not whether the values survive a restart. Both must be `true` to manage OAuth from the Admin Panel.
...
**When a provider does not say when the token expires:** most return `expires_in` or `expires_at`, and Open WebUI trusts whichever it gets. If neither is present, what happens next depends on whether the response included a refresh token. With one, the token is assumed to last an hour and is refreshed on that basis, and a warning names the provider in the logs. Without one there is nothing to refresh with, so inventing a short expiry would only end the session for no reason; the token is treated as non-expiring instead and the session survives. This is why a provider that issues long-lived tokens and no refresh token no longer drops its users after an hour.
...
To enable this feature, set `ENABLE_OAUTH_TOKEN_EXCHANGE=true` and list the OAuth clients you trust in `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS`. See the [`ENABLE_OAUTH_TOKEN_EXCHANGE`](/reference/env-configuration#enable_oauth_token_exchange) environment variable documentation for details on usage, request/response examples, and security considerations.

Like a browser sign-in, an exchange re-applies the user's role from the provider's claims and reconciles their group memberships when `ENABLE_OAUTH_GROUP_MANAGEMENT` is on. It does not refresh the profile: `OAUTH_UPDATE_NAME_ON_LOGIN`, `OAUTH_UPDATE_EMAIL_ON_LOGIN` and `OAUTH_UPDATE_PICTURE_ON_LOGIN` govern the browser sign-in alone, so a name, email or picture changed at the provider reaches Open WebUI when the user next signs in through the interface rather than on their next exchange.

:::danger

A successful exchange issues a session as the user the token identifies, with that user's role. Because a provider's userinfo endpoint cannot reveal which OAuth client a token was issued to, the endpoint must be told which clients you trust. With `OAUTH_TOKEN_EXCHANGE_TRUSTED_CLIENT_IDS` unset the issuing client is not checked at all, so a token from any application your users authorise is accepted; with it set, any holder of a token from a listed client can obtain a session as the corresponding user. `OAUTH_ALLOWED_DOMAINS` is still enforced here, but it does not help, because an impersonated user is a legitimate member of an allowed domain.

Resolving the issuing client requires RFC 7662 token introspection, which Google, Microsoft Entra ID, GitHub and Feishu do not implement, so the restriction cannot be applied on those providers.
...
:::info Providers that add their own parameters to the ID token

Some providers add a vendor-specific parameter of their own to the header of the ID token they issue. Open WebUI ignores parameters it does not recognize there, so sign-in through such a provider completes normally, and the token's signature and signing algorithm are still verified as before.

Earlier versions rejected the whole token instead, so the login ended at the callback with `The email or password provided is incorrect` in the browser and `Unsupported {'app_id'} in header` in the server log. If you see that pair, update Open WebUI.
...
When that login does change the role, Open WebUI ends the account's other live WebSocket connections, so an old tab cannot keep acting on the previous role. Those browsers reconnect and re-authenticate on their own. See [When a Role Change Takes Effect](/features/authentication-access/rbac/roles#when-a-role-change-takes-effect).
...
Each membership the sync adds or removes, and each group `ENABLE_OAUTH_GROUP_CREATION` creates, emits an event carrying `source: 'oauth'`, so the login path is visible to [Event functions](/features/extensibility/plugin/functions/event) and to anything auditing group membership.
...
The account created on a person's first request is given an identifier derived from their email address rather than a random one, so several requests arriving together for someone who has never signed in before all resolve to the same account. Previously each of those requests could create its own, leaving duplicate users for the same person, which is a realistic way for a first visit to begin when a page loads several resources at once.
...
You can use the `WEBUI_AUTH_TRUSTED_ROLE_HEADER` environment variable to control a user's role via the authenticating reverse proxy. Set this variable to the name of the HTTP header that will contain the desired role for the authenticated user.

- The header value must be one of `admin`, `user`, or `pending` (case-insensitive, trimmed).
- When the header is present and valid, the user's role is updated on every sign-in to match the header value.
- If the header contains an invalid value, the role is not changed and a warning is logged.
- If the header is not present or empty, the user's existing role is left unchanged.
- When the header does change the role, the account's live WebSocket connections are ended so the previous role cannot linger in an open tab. Those browsers reconnect and re-authenticate by themselves. See [When a Role Change Takes Effect](/features/authentication-access/rbac/roles#when-a-role-change-takes-effect).
