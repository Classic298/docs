...
| | |
| :--- | :--- |
| 📄 **13 vector databases** | ChromaDB and PGVector (officially maintained), plus non-core integrations: Qdrant, Milvus, OpenSearch, Elasticsearch, and more |
...
![The knowledge base list](/images/workspace/knowledge-list.png)
...
![Files inside a knowledge base, with search and sort](/images/workspace/knowledge-detail.png)
...
| **Result cap** | Top K (default 5) | 50 matches ([`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches)) |
...
- **Character pagination**: `offset` + `max_chars` (default `10000`, hard cap `100000`; both configurable with [`VIEW_FILE_DEFAULT_MAX_CHARS`](/reference/env-configuration#view_file_default_max_chars) and [`VIEW_FILE_MAX_CHARS`](/reference/env-configuration#view_file_max_chars)). Best for streaming through a long document; the response includes `next_offset` when the file is truncated.
...
| `ls -t`, `ls -S`, `ls -r` | Sort the listing: newest first, largest first, or reverse whichever order is in effect |
...
- **Output is bounded per call.** One command returns at most 30,000 characters ([`KB_EXEC_MAX_OUTPUT_CHARS`](/reference/env-configuration#kb_exec_max_output_chars)), whatever produced them, and the model is told to narrow the command when it hits the limit. This is a single budget for the whole pipeline, so a `cat` of a large file and a wide `grep` are capped the same way.
- **A wide `grep` is refused rather than run.** Searching more than 200 files ([`KB_EXEC_MAX_GREP_FILES`](/reference/env-configuration#kb_exec_max_grep_files)) returns a prompt to scope the search to a directory or glob, and `grep` returns at most 50 matching lines ([`KNOWLEDGE_GREP_MAX_MATCHES`](/reference/env-configuration#knowledge_grep_max_matches)), the same cap `grep_knowledge_files` and `grep_chat_files` use.
...
2. Click **Create** in the Workspace header and give it a name and description.
